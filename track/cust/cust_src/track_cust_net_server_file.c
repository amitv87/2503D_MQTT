...
static void resend_FE_03()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(G_parameter.agps.sw && track_cust_gps_status() == GPS_STAUS_SCAN) /* 无经纬度，但有EPO文件，尝试注入 */
            {
...
static void track_cust_check_zkw_gps_file(kal_uint8* zkw_buf, U32 size)
{
	...
	LOGD(L_APP, L_V3, "==[zkw 星历数据存入文件] %d %d file_size %d", ret, fs_result,  wSize);
...
static void track_cust_zkw_gps_epodata(kal_uint8* zkw_buf, U32 size)
{
	...
	if(!zkw_epo)
	{
		LOGD(L_APP, L_V3, "ERRO 中科微GPS EPO数据存储失败!!!!");
		if(count>3)
		{
			LOGD(L_APP, L_V3, "无法申请内存重启");
	...
	if(G_realtime_data.zkw_eph_datatime!=0 && (appid==2 || appid==3 ) && G_parameter.agps.sw)
	{
		...
		if(timep > G_realtime_data.zkw_eph_datatime)
		{
			...
			if(t2>ZKW_EPH_TIMEOUT_MIN)
			{
				...
				LOGD(L_APP, L_V3,"星历已存%d分钟，需要更新!!!",t2);
				return KAL_FALSE;
			}
			else
			{
				if(track_zkw_eph_assistant_param()>=0)
				{
					if(zkw_epo)
					{
						...
						LOGD(L_APP, L_V3, "主动注入星历数据!!!");
					}
					else
					{
						LOGD(L_APP, L_V3,"buffer 无星历数据");
						return KAL_FALSE;
					}
				}
...
static void recv_login(kal_uint8 *data, kal_uint16 len)
{
		...
		else
		{
	        ...
	        if(G_realtime_data.zkw_eph_datatime!=0)
	        {
				...
				else
				{
					...
					if(t2>ZKW_EPH_TIMEOUT_MIN)
					{
						LOGD(L_APP, L_V4,"星历已存%d分钟，需要更新!!!",t2);
						track_cust_delete_zkw_file();					
						track_cust_release_zkw_buf();
						updateforagpsfile |=2;
					}
    ...
    if(data[14] == 1)
    {
        ...
        track_cust_ota_cmd();
        LOGD(L_APP, L_V4, "OTA升级向单片机发07升级指令延迟15分钟关机!!!");
#endif
    }
    ...
    else if(data[14] == 5)
    {
            ...
            && !track_cust_check_atgm_update_image_exsist())
        {
            ...
            LOGD(L_APP, L_V4, "ATGM有更新升级提供");
        }
        else
        {
            LOGD(L_APP, L_V4, "ATGM不更新%d %d %d", 
    ...
    else if(data[14] == 4) /* MCU */
    {
        ...
        if(value1 >= 360)
        {
            updatefiletype = data[14];  // 0x04 为该终端MCU更新  
            }
        #else
        updatefiletype = data[14];  // 0x04 为该终端MCU更新  
        #endif
		...
		#if defined (__JM81__)
        	LOGD(L_APP, L_V4, "MCU单片机软件升级!!!");
		#else
	        track_cust_ota_cmd();
	        LOGD(L_APP, L_V4, "MCU单片机软件升级,延迟15分钟关机!!!");
...
static void recv_file_content(kal_uint8 *data, kal_uint16 len)
{
    ...
    else
    {
        ...
        else if(nowUpdateFile == 8  && (updateforagpsfile & 0x02))
        {
			...
			{
            	LOGD(L_APP, L_V4, "中科微卫星星历注入");
            	U_PutUARTBytes(2, buf, file_size);//GPS_UART_PORT
			}
        }
        LOGD(L_DRV, L_V4, "中科微卫星星历注入条件%d,%d,%d", nowUpdateFile, track_cust_gps_status(), updateforagpsfile);
        #endif
        if(buf)
        {
            //   Ram_Free(buf);
...
void track_cust_server2_decode_packet(kal_uint8 *data, kal_uint16 len)
{
    ...
    #if defined(__ZKE_EPO__)
    else if(packet_type == 1 || packet_type == 2 || packet_type == 7 || packet_type == 8) /* 文件接收过程 */
    #else
