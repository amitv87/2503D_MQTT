...
void obd_poweron(kal_uint32 on)
{
    static U8 ss = 2;
    kal_int8 ret;
    LOGD(L_OBD, L_V6, "last:%d, %d", ss, on);
    if(on > 20)
    {/*开*/
        if(ss != 1)
        {
            ...
            if(ret != 0 && ret != 1)
            {
                LOGD(L_OBD, L_V1, "ERROR1 ret:%d", ret);
            }
            else ss = 1;
    ...
    else if(on < 20)
    {
        ...
        {/*关*/
            ...
            if(ret != 0 && ret != 1)
            {
                LOGD(L_OBD, L_V1, "ERROR2 ret:%d", ret);
            }
            else ss = 0;
        }
    }
...
static void obd_sleep_turnon(void)
{
    LOGD(L_OBD, L_V2, "obd_acc_status=%d, obd_type=%d", obd_acc_status, obd_type);
    if(obd_acc_status > 0)
    {
        tr_start_timer(TRACK_CUST_OBD_SLEEP_TIMER_ID, 60000, obd_sleep_turnon);
    }
...
static void obd_get_data(kal_uint8 type, char *obd_error_id)
{
    ...
    LOGD(L_OBD, L_V5, "%d", type);
    {
        ...
        if(time.nYear < 2014)
        {
            ...
            LOGD(L_OBD, L_V5, "设备时间未同步，OBD数据暂不上传");
            return;
        }
    }
    for(i=0; i<OBD_PARAMETER_MAX; i++)
    {
        if(obd_datas[i].valid && ((type & 0x01) == 0 || ((type & 0x01) == 1 && obd_datas[i].update_valid)))
        {
            ...
            {
                LOGD(L_OBD, L_V4, "{%d,%d}%s,%s=%X", i, obd_datas[i].source, 
            ...
            else
            {
                ...
                LOGD(L_OBD, L_V4, "{%d,%d}%s,%s=%0.2f(%X)", i, obd_datas[i].source, 
...
static void obd_obdversion_ok(void)
{
    ...
    {
        mode_update = 0;
        LOGD(L_OBD, L_V4, "OBD: 升级成功重新连接服务器提交新版本!");
...
static void obd_gercar_ok(void)
{
    ...
    LOGD(L_OBD, L_V4, "GETCAR:%s,%d", obd_parameters[1], car);
...
static void obd_work_ok(void)
{
    ...
    track_stop_timer(TRACK_CUST_OBD_RESTART_TIMER_ID);
    track_cust_obd_write(str, strlen(str));
    LOGD(L_OBD, L_V4, "obd_work_ok,%s,%s", obd_parameters[1], obd_parameters[2]);
    obd_type = 1;
    obd_start_status = KAL_TRUE;
    ver = track_fun_trim(obd_parameters[2]);
    len = strlen(ver);
...
{
    LOGD(L_OBD, L_V1, "obd_opendata_ok");
...
static void obd_decode_data(void)
{
    ...
    kal_uint8 source;
    kal_uint32 value;
    LOGD(L_OBD, L_V1, "size:%d", sizeof(obd_datas));
    for(i=1; i<obd_parameters_count-1; i++)
    {
        ...
        LOGD(L_OBD, L_V1, "{%d,%d,%d}%s, %s", i, j, obd_datas[j].source, obd_parameters[i], tmp);
    }
    LOGD(L_OBD, L_V1, "------------------------------------------------------------------");
}
...
static void obd_vin(void)
{
    ...
    {
        LOGD(L_OBD, L_V5, "VIN:%s", obd_parameters[1]);
    }
    if(obd_type == 1)
    {
...
static void obd_obdsleep_ok(void)
{
    LOGD(L_OBD, L_V1, "obd_obdsleep_ok");
    track_stop_timer(TRACK_CUST_OBD_CXALL_TIMER_ID);
...
{
    LOGD(L_OBD, L_V1, "obd_closedata_ok");
    obd_obdsleep_on();
}
...
static void obd_event_up(void)
{
    ...
    LOGD(L_OBD, L_V5, "%d,%d", value1, value2);
    obd_event_up_ok();
}
...
static kal_int32 obd_read_file_data(void)
{
    ...
    if(fp < 0)
    {
        LOGD(L_OBD, L_V1, "FS_Open Error:%d", fp);
        return fp;
    }
    ...
    if(size > 0)
    {
        ...
        if(obdfiledatabuf == NULL)
        {
            LOGD(L_OBD, L_V1, "Error: Ram_Alloc %d NULL!!", obdfilefilesize);
            return -1;
        }
    ...
    else
    {
        ...
        LOGD(L_OBD, L_V1, "Error: obdfilefilesize=%d", size);
        return -2;
    }
    ...
    while(len > 0)
    {
        ...
        {
            OTA_FS_Close(fp);
            LOGD(L_OBD, L_V1, "fs_result:%d, %d, %d", fs_result, readsize, len);
            return fs_result;
        }
        len -= readsize;
        p += readsize;
...
{
    U32 status = (U32)arg;
    LOGD(L_OBD, L_V3, "%d,%d", status, obd_acc_status);
    switch(status)
    {
        case 0:
        {
...
void track_cust_obd_write(kal_uint8 *data, int len)
{
    ...
    if(len > 92)
    {
        LOGD(L_OBD, L_V1, "Error:too long!");
        return;
    }
    memcpy(buf, data, len);
    while(1)
...
void track_cust_obd_update_fireware(U8 *data, U32 len)
{
    ...
    {
        LOGD(L_OBD, L_V4, "ERROR: 无可供升级OBD的数据");
    }
}
...
{
    LOGD(L_OBD, L_V5, "CMD: %s", cmd->pars[1]);
...
static void obd_send_cmd_md5_check(void)  /*升级数据文件内容校验数据命令*/
{
    ...
    obd_poweron(29);
    track_drv_uart1_write_data(data, sizeof(data));
    LOGH(L_OBD, L_V1, data, sizeof(data));
...
static void obd_send_cmd_update_data(void)  /*升级数据发送命令*/
{
    ...
    if(obdfiledatabuf == NULL)
    {
        LOGD(L_OBD, L_V5, "Error: 用于升级的OBD数据未准备好!");
    ...
    LOGD(L_OBD, L_V1, "update_file_block:%d, datalen:%d, obdfilefilesize:%d", update_file_block, datalen, obdfilefilesize);
    ...
    if(packet_buf == NULL)
    {
        LOGD(L_OBD, L_V1, "Ram_Alloc NULL!!");
        return;
    }
...
static void obd_cmd_hex(kal_uint8 *data, int len)
{
    ...
    LOGD(L_OBD, L_V5, "收到二进制数据%d 命令字: %0.2X %0.2X", len, data[0], data[1]);
        ...
        if(buf[j] == 'A' && buf[j+1] == 'T' && buf[j+2] == '^' && buf[j+3] == 'D' && buf[j+4] == '=')
        {
            ...
            if(end != NULL)
            {
                ...
                if(*(end - 5) == ',')
                {
                    ...
                    {
                        *end = 0;
                        //LOGH(L_OBD, L_V2, crc, 4);
                        LOGD(L_OBD, L_V1, "重复，跳过(%s)", start);
                        g_tick_new = OTA_kal_get_systicks();
                        continue;
                    }
                    memcpy(crc, end-4, 4);
                ...
                LOGD(L_OBD, L_V1, "[%d] %s", obd_parameters_count, start + 5);
                track_stop_timer(TRACK_CUST_OBD_START_TIMER_ID);
                for(i = 0; i < size; i++)
                {
                    ret = strcmp(obd_parameters[0], OBD_Data[i].cmd_string);
                    //LOGD(L_OBD, L_V9, "ret=%d|%s,%s", ret, obd_parameters[0], OBD_Data[i].cmd_string);
                    if(!ret)
                    {
                        LOGD(L_OBD, L_V9, "[%d], %s", i, OBD_Data[i].cmd_string);
                        if(OBD_Data[i].func_ptr != NULL)
                        {
                            OBD_Data[i].func_ptr();
                            LOGD(L_OBD, L_V1, "run [%d], %s", i, OBD_Data[i].cmd_string);
                            g_tick_new = OTA_kal_get_systicks();
                        }
                        break;
                    }
