...
#if defined (__GW100__)
        //GW100 强制要求GPS在SOS状态下工作5分钟,由于其他项目并未明确说明
        //故此处专门为GW100强制调用一次
        track_cust_gps_work_req((void *)10);
#endif /* __GW100__ */
...
kal_bool  is_host_number(char* number)
{
    ...
    if(host_no != sms_no)
    {
        LOGD(L_CMD, L_V5, "车主号码不匹配");
        return FALSE;
    }
    else
    {
        LOGD(L_CMD, L_V5, "车主号码匹配");
        return TRUE;
    }
}
#endif
...
/*录音时不能通话       --    chengjun  2017-10-09*/
                ...
                {
                    LOGD(L_APP, L_V5, "任意号码可以监听. MonPermit = %d", G_parameter.Permission.MonPermit);
                }
                else if(G_parameter.jc_mode == 0 && G_parameter.Permission.CallPermit == 3)
                {
                    LOGD(L_APP, L_V5, "任意号码可以接听. MonPermit = %d", G_parameter.Permission.CallPermit);
        ...
        case TR_CALL_DISCONNECT:
            {
                ...
                LOGD(L_APP, L_V5, "挂断 e_tick=%d次", e_tick);
                if(is_host)
                {
                    is_host = FALSE;
