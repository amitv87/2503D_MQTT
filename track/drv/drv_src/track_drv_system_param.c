...
#if 0
/*我只是用它判断大结构体的数据是否对的,不对的话，该取用谁，反正最后在写入的时候，都会全部重写数据区*/
void track_drv_disk1_check_crc(kal_uint8* data,kal_uint32 crc_begin)
{
    ...
    if(data[crc_begin] == backup_buf[crc_begin] && data[crc_begin+1] ==backup_buf[crc_begin+1])//与备份区校验
    {
        if(data[crc_begin] == 0xFF && data[crc_begin+1]==0xFF)//备份区也没校验值,终端是首次开机吧
        {
        ...
        {
            if(data[crc_begin] !=((crc_page >> 8) & 0x00FF) || data[crc_begin+1]!=(crc_page & 0x00FF))//数据区校验不过
            {
                if(backup_buf[crc_begin]!=((crc_backup >> 8) & 0x00FF) || backup_buf[crc_begin+1]!=(crc_backup & 0x00FF))//备份区校验不过
                {
...
kal_bool track_drv_check_disk1_nvram(kal_uint8 type)
{
    ...
    if(t_imp->build_date[0] == 0xFF || t_imp->build_time[0] == 0xFF) /* 这两个变量在目标备份区未初始化时，全是0xFF，日志按字符打印显示会过长可能导致重启 */
...
kal_bool Track_nvram_write(nvram_lid_enum LID, kal_uint16 rec_index, void *buffer, kal_uint32 buffer_size)
{
    ...
    switch(LID)
    {
        ...
        case DISK1_BACKUP_LID:
            ...
            if(!writeValid)
            {
                LOGD(L_DRV, L_V5, "临时禁止写入数据! LID:%d,bufsize:%d, address:ox%x, ssize:%d, writeValid:%d", LID, buffer_size, sds[LID].address, sds[LID].size, writeValid);
                return KAL_TRUE;
            }
            else
            {
