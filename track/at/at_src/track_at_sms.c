...
void track_sms_send_ctx(void)
{
    ...
    if(ssp.sms_size)
    {
        ...
        {
            //snprintf(vs_sms.at_head, sizeof(vs_sms.at_head), "CMGS:");

#if defined( __GT530__)
            track_drv_encode_exmode (0x77, 0x0101, 0x95, ssp.sms_ctx, ssp.sp_size);     
#else
...
void track_sms_sender(sms_msg_struct * sms_send)
{
    ...
    if(sms_send->nocount > 0 && sms_send->ctxlen > 0)
    {
        ...
        for(index = 0; index < sms_send->nocount; index++)
        {
            ...
            memset(&sspdu, 0, SSP_SIZE);

            if(sms_send->codetype)//将ascii转换成unicode传给4G模块
            {
                //转换成unicode
                encode_msg.ictxlen = track_fun_ascii2ucs2(encode_msg.encode_ctx, sms_send->ctx, sms_send->ctxlen);
                
                inumlength = sprintf(sspdu.sms_ctx, "SMSMO,%s,%d,", sms_send->numbers[index], encode_msg.ictxlen);
                memcpy(&sspdu.sms_ctx[inumlength], encode_msg.encode_ctx, encode_msg.ictxlen);
            sspdu.sms_size = inumlength + encode_msg.ictxlen;
            }
            else
            {
                inumlength = sprintf(sspdu.sms_ctx, "SMSMO,%s,%d,", sms_send->numbers[index], sms_send->ctxlen);
                memcpy(&sspdu.sms_ctx[inumlength], sms_send->ctx, sms_send->ctxlen);
                sspdu.sms_size = inumlength +  sms_send->ctxlen;
            }

            sspdu.sms_dir = sms_send->smsdir;
            sspdu.sms_len = sspdu.sms_size/2;
        ...
        {
            #if 1
            if(strstr(track_drv_get_imsi(0), "46004") > 0 || strstr(track_drv_get_imsi(0), "46006") > 0)
            {
				...
				sms_send->ctxlen = encode_msg.ctxlimit;
...
void track_sim_clean_reveicebox(void)
{
    ...
    tr_start_timer(TRACK_CUST_CLEAN_SMS_TIMER, 60 * 1000, track_sim_clean_read_sms_reveicebox);
}
...
void test_sms(void * mm)
{
   ...
   // char * yixi = "13818604300";
    ...
    else if(m == 9)
    {
        memset(&ssms, 0, sizeof(sms_msg_struct));
        ssms.codetype = KAL_FALSE;
        ssms.ctxlen = sizeof(ssw);
        ssms.nocount = 1;
        ssms.numbers[0] = test_sms_number(NULL);
        m = sprintf(yi, "SMSMO,%s,%d,", test_sms_number, ssms.ctxlen);
        memcpy(&yi[m], ssw, sizeof(ssw));
        ssms.ctxlen += m;
        ssms.ctx = yi;
        track_drv_encode_exmode (0x77, 0x0101, 0x95, ssms.ctx, ssms.ctxlen);     

        LOGH(L_CMD, L_V5, yi, m + ssms.ctxlen);
        
        memset(&ssms, 0, sizeof(sms_msg_struct));
        memset(&yi, 0, sizeof(yi));
        ssms.codetype = KAL_FALSE;
        ssms.ctxlen = sizeof(ssw1);
        ssms.nocount = 1;
        ssms.numbers[0] = test_sms_number(NULL);
        m = sprintf(yi, "SMSMO,%s,%d,", test_sms_number(NULL), ssms.ctxlen);
        memcpy(&yi[m], ssw1, sizeof(ssw1));
        ssms.ctxlen += m;
        ssms.ctx = yi;
        track_drv_encode_exmode (0x77, 0x0101, 0x95, ssms.ctx, ssms.ctxlen );    
        ...
        memset(&yi, 0, sizeof(yi));
        ssms.codetype = KAL_FALSE;
        ssms.ctxlen = sizeof(org);
        ssms.nocount = 1;
        ssms.numbers[0] = test_sms_number(NULL);
        m = sprintf(yi, "SMSMO,%s,%d,", test_sms_number(NULL), ssms.ctxlen);
        memcpy(&yi[m], org, sizeof(org));
        ssms.ctxlen += m;
        ssms.ctx = yi;
        track_drv_encode_exmode (0x77, 0x0101, 0x95, ssms.ctx, ssms.ctxlen ); 
        LOGD(L_CMD, L_V5, "======================");
    ...
    else if(m == 204)
    {
        ...
        LOGS("Current GPS Module is:GPS_G3333");
#elif defined(__GPS_G3337__)
        LOGS("Current GPS Module is:GPS_G3337");
#elif defined(__GPS_G7020__)
        LOGS("Current GPS Module is:GPS_G7020");
#elif defined(__GPS_MT5631B__)
        LOGS("Current GPS Module is:GPS_MT5631B");
#elif defined(__GPS_M10382__)
        LOGS("Current GPS Module is:GPS_M10382");
#elif defined(__GPS_MT3336__)
        LOGS("Current GPS Module is:GPS_MT3336");
#elif defined(__GPS_MT3332__)
        LOGS("Current GPS Module is:GPS_MT3332");
#else
        LOGS("Current GPS Module is:XXXXXXX");
#endif /* __GPS_G3333__ */
    }
    else if(m == 205)
    {
    ...
    {
        track_drv_sleep_enable(SLEEP_MOD_MAX, KAL_FALSE);
    }
    else if(210 <= m && m < 220)
    {
        track_drv_test_disk1(m);
    }
    else if(220 == m)
    {
