...
void track_send_at_accept_call(void)
{
    ...
    track_drv_encode_exmode(0x77, 0x0101, 0x91, "ATA", 3);
#else
...
#if defined( __GT530__)
    track_drv_encode_exmode(0x77, 0x0101, 0x91, "HANGUP", 6);
#else
...
#if defined( __GT530__)
    nsize = snprintf(call_cmd, 25, "ATD,%s", number);
#else
...
#if defined( __GT530__)
    track_drv_encode_exmode(0x77, 0x0101, 0x91, call_cmd, nsize);
#else
...
void track_press_key_dial_112(void)
{
    LOGS("Dail \"112\"");
#if defined( __GT530__)
    track_drv_encode_exmode(0x77, 0x0101, 0x91, "ATD,112", 7);
#else
...
#if defined( __GT530__)
    if(call_msg.call_state == TR_CALL_DISCONNECT)
    {
            call_msg.call_type = CALL_FREE;
    }
#endif /* __GT530__ */
...
{/*MSG_ID_MMI_CC_DIAL_IND  track_l4c_cc_exe_call_setup_lrsp*/
    ...
    if(CALL_SETUP_LRSP == setup_type)
    {
        ...
        {
            LOGS("ATD,%s",(ptr->cause == 0 ? "OK": "FAIL"));
        }
#endif /* __GT530__ */
    ...
    else if(CALL_SETUP_ATA == setup_type)
    {
        ...
        if(track_is_testmode())
        {
            LOGS("ATA,%s",(ptr->cause == 0 ? "OK": "FAIL"));
        }
#endif /* __GT530__ */
    }
    else
