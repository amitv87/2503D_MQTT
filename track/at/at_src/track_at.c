...
static kal_bool got_uart1_cmd = KAL_FALSE;//本次开机是否接收过串口1的指令
...
void track_at_set_got_uart1_cmd(void)
{
    got_uart1_cmd = KAL_TRUE;
}
...
void track_uart_data_analysis(kal_int8 type, char *rsp_data, kal_uint32 len)
{
    ...
    if(got_uart1_cmd)
    {
        LOGS("%s", rsp_data);//防止充电数据线RX TX短路导致一直重启
    }
#else
    LOGS("%s", rsp_data);
#endif
    ...
    LOGD(L_CMD, L_V1, "UART0无规则数据");
    if(track_drv_uart1_mux_read(rsp_data, len))
    {
        return;
    }
...
{
    //extern void track_drv_get_sim_sms_callback();

    //tr_start_timer(TRACK_AT_HAND_FREE_TIMER_ID, 5000, track_at_set_handfree); //06B硬件要求需要切换免提
    //tr_start_timer(TRACK_SMS_AUTO_IN_TIMER_ID, 20 * 1000, track_sms_set_auto_to_serial);
    //tr_start_timer(TRACK_SIM_READ_SB_TIMER_ID, 20*1000, );
