 ...
 *
 *             app:     1=GPS;2=服务器;3=AGPS服务器;4=单片机外设  
 *
 *  modification history
 * ----------------------------------------
 * v1.0  , 11-09-2012,  Cml  written
...
static int track_fun_split_small2big(kal_uint16 a[], int low, int high)
{
    ...
    for (;;)
    {
    	while (low<high&&part_element <= a[high])high--;//从大到小则改为>=
    	if (low >= high)break;
    	a[low++] = a[high];
    	while (low<high&&part_element >= a[low])low++;//从大到小则改为<=
    	if (low >= high)break;
    	a[high--] = a[low];
    }
    a[high] = part_element;
