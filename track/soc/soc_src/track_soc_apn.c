...
{
    LOGD(L_SOC, L_V4, "");
    if(track_nvram_alone_parameter_read()->apn.custom_apn_valid && strlen(track_nvram_alone_parameter_read()->apn.apn) > 0)
    {
        track_fun_strncpy(apn_use.apn, track_nvram_alone_parameter_read()->apn.apn, MAX_GPRS_APN_LEN);
        track_fun_strncpy(apn_use.username, track_nvram_alone_parameter_read()->apn.username, MAX_GPRS_USER_NAME_LEN);
        track_fun_strncpy(apn_use.password, track_nvram_alone_parameter_read()->apn.password, MAX_GPRS_PASSWORD_LEN);
...
char *track_socket_apn_view(void)
{
    ...
    if(track_nvram_alone_parameter_read()->apn.custom_apn_valid && strlen(track_nvram_alone_parameter_read()->apn.apn) > 0)
    {
        snprintf(tmp, 200, "Currently use APN:%s, UserName:%s, Password:%s\r\nCurrently set APN:%s, UserName:%s, Password:%s", 
            ...
            track_nvram_alone_parameter_read()->apn.apn, track_nvram_alone_parameter_read()->apn.username, track_nvram_alone_parameter_read()->apn.password);
    }
    else
    {
        snprintf(tmp, 200, "Currently use APN:%s, UserName:%s, Password:%s\r\nCurrently set APN:%s, UserName:%s, Password:%s", 
...
nvram_GPRS_APN_struct *track_socket_get_use_apn(void)
{
    ...
    {
        apn_use.custom_apn_valid = track_nvram_alone_parameter_read()->apn.custom_apn_valid;
        track_fun_strncpy(apn_use.apn, track_nvram_alone_parameter_read()->apn.apn, MAX_GPRS_APN_LEN);
        track_fun_strncpy(apn_use.username, track_nvram_alone_parameter_read()->apn.username, MAX_GPRS_USER_NAME_LEN);
        track_fun_strncpy(apn_use.password, track_nvram_alone_parameter_read()->apn.password, MAX_GPRS_PASSWORD_LEN);
        LOGD(L_SOC, L_V4, "使用自定义 APN:%s, username:%s, password:%s", apn_use.apn, apn_use.username, apn_use.password);
...
void track_socket_check_apn_diskdata()
{
    ...
    port = track_nvram_alone_parameter_read()->ams_server.server_port;
  	LOGD(L_APP, L_V5, "(%s===%s)%d====%d", nvramdata.server,server, nvramdata.port,port);
