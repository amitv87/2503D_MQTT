...
static void cmd_entry_test_mode(CMD_DATA_STRUCT * Cmds)
{
    ...
    if(Cmds->part == 0)
    {
        ...
        if(track_status_test(2) == KAL_TRUE)
        {
            ...
            OTA_track_custom_fake_cell_feature_enable(0);//测试模式关闭防伪基站
...
static CMD_ENGINE_STRUCT  cmd_table[] =
{
    ...
    {"CS",           cmd_test_sms_call,              TRUE},      //临时测试使用

    /* 工厂测试相关指令 */
    {"GPSTC",        cmd_GPS_PassThrough_mode,       TRUE},      //GPS 透传
    {"GPS_OFF",      cmd_entry_gpsoff_mode,          TRUE},      //测试模式
    {"TEST",         cmd_entry_test_mode,            TRUE},      //测试模式
    //{"BMT",          cmd_get_battery_status,         TRUE},      //电池电压查询
    {"ID",           cmd_Chip_id,                    TRUE},

#if defined (__AUTO_TEST__)
#if !defined(__GT740__)||!defined(__GT420D__)
//  {"TEST_AUTO",    cmd_entry_autotest_mode,        TRUE},      //自动测试模式
    {"GPSCOLD",      cmd_entry_gpscold_mode,         TRUE},      //自动测试模式
    {"RELAY_L",      cmd_entry_relay_l_mode,         TRUE},      //继电器吸合
    {"RELAY_H",      cmd_entry_relay_h_mode,         TRUE},      //继电器断开
    {"SENSOR_TEST",  cmd_entry_sensortest_mode,      TRUE},      //自动化测试下SENSOR测试
    {"SENSOR_OFF",   cmd_entry_sensoroff_mode,       TRUE},      //自动化测试下SENSOR测试
    {"FSTTIME",      cmd_entry_fsttime_mode,         TRUE},      //查询GPS冷启动后搜星时间
    {"GPIO",         cmd_entry_gpio_mode,            TRUE},      //设置GPIO口
    {"SENSOR_L",     cmd_entry_sensorlevel_mode,     TRUE},      //设置SENSOR等级
...
#endif /* __WATCH_DOG__ */
#if defined(__AUDIO_RC__)
    {"SPEAKER_ON",   cmd_speaker_on,                 TRUE},      //检查SIM卡是否存在
    {"SPEAKER_OFF",  cmd_speaker_off,                TRUE},      //检查SIM卡是否存在
#endif /* __AUDIO_RC__ */
...
//  {"LEVEL",        cmd_set_sensor_level,           TRUE},      //Sensor  激活GPS 灵敏度
//  {"COLD",         cmd_set_GPS_cold_start,         TRUE},      //GPS冷启动
//  {"FACTORY",      cmd_restore_factory_default,    TRUE},      //恢复出厂设置
//  {"APN",          cmd_set_APN,                    TRUE},      //APN参数设置
//  {"SERVER",       cmd_set_server,                 TRUE},      //服务器参数设置
