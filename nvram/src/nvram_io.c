...
kal_bool nvram_external_default_data(nvram_lid_enum LID)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (nvram_reset_one_data_item(ldi, 1, ldi->total_records) == NVRAM_IO_ERRNO_OK) {
		return KAL_TRUE;
    } else {
        return KAL_FALSE;
    }
    ...
    switch (reset_category)
    {
        ...
        case NVRAM_RESET_FACTORY:
        {
            if (nvram_ptr->state != NVRAM_STATE_READY)/* init stage */
            {
                ...
                nvram_set_restore_factory_flag(NVRAM_RESTORE_FALSE);//DT??nvram2??¨¹?y3¡êD¡ä¨¨?¦Ì??¨º¨¬a
#endif
            }
            else
            {
