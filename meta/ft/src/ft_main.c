 ...
 *   None
 *******************************************************************************/
static void FT_InitCalibrationData(task_entry_struct *task_entry_ptr)
{
    {
        ilm_struct* ptr_ilm;
        nvram_startup_req_struct *ptrMsg;
        ptr_ilm = FT_ALLOC_OTHER_MSG( sizeof(nvram_startup_req_struct) );
        /* if ptrMsg != NULL*/
        ptrMsg = (nvram_startup_req_struct *)ptr_ilm->local_para_ptr;
        ptrMsg->poweron_mode=NVRAM_POWERON_NORMAL;
        /* FT_SEND_MSG(src_mod, dest_mod, sap_id, msg_id, ilm_ptr) */
        FT_SEND_MSG(MOD_FT, MOD_NVRAM, PS_NVRAM_SAP, MSG_ID_NVRAM_STARTUP_REQ, ptr_ilm);
    }
}
...
/* under construction !*/
            ...
            {
                FT_L4Audio_Operation(ptrMsg);
                break;
            }
            case FT_L4AUD_REQ_EX_ID:
            {
                FT_L4AudioEx_Operation(ptrMsg);
                break;
            }
#endif // #if (!defined(__SMART_PHONE_MODEM__))
            /********************************************
            ...
            case FT_NVRAM_GET_DISK_INFO_REQ_ID:
            {
                ...
                ptr_ilm = FT_ALLOC_OTHER_MSG( sizeof(FT_NVRAM_GET_DISK_INFO_CNF) );
            ...
            {    ilm_struct *ptr_ilm;
                ...
                // store nvram token id, because any of NVRAM command won't return immediately.
                ft_gl_nvram_token = ft_header->token;
                ptr_ilm = FT_ALLOC_OTHER_MSG( sizeof(nvram_reset_req_struct) );
                /* if ptr_ilm != NULL*/
            ...
            case FT_NVRAM_LOCK_REQ_ID:
            {
                ...
                ft_gl_nvram_token = ft_header->token;
                ptr_ilm = FT_ALLOC_OTHER_MSG( sizeof(nvram_set_lock_req_struct) );
                /* if ptr_ilm != NULL*/
            ...
            {
                // store BT token id
                ft_gl_bt_token = ft_header->token;
                FT_BT_Operation((FT_BT_REQ *)ptrMsg->local_para_ptr, ptrMsg->peer_buff_ptr);
                break;
            }
            ...
            /********************************************
             *
             *  FT task FM Radio Command
             *
             ********************************************/
            case FT_FM_REQ_ID:
            {
                FT_Fm_Operation(ptrMsg);
                break;
            }
            /********************************************
             *
             *  TDMB task utility command
             *
             ********************************************/
#if defined(__TDMB_SUPPORT__)
            case FT_TDMB_REQ_ID:
            {
                FT_TDMB_Operation(ptrMsg);
                break;
            }
#endif // #if defined(__TDMB_SUPPORT__)
            /********************************************
             *
             *  CMMB task utility command
             *
             ********************************************/
#if defined(__CMMB_SUPPORT__)
            case FT_CMMB_REQ_ID:
            {
                FT_CMMB_Operation(ptrMsg);
                break;
            }
#endif // #if defined(__CMMB_SUPPORT__)
            /********************************************
             *
             *  mATV utility command
             *
             ********************************************/
#if defined(__ATV_SUPPORT__)
            case FT_MATV_CMD_REQ_ID:
            {
                FT_MATV_Operation(ptrMsg);
                break;
            }
            ...
            case FT_CCT_REQ_ID:
            {
                FT_CCT_Operation((FT_CCT_REQ *)ptrMsg->local_para_ptr, ptrMsg->peer_buff_ptr);
                break;
            }
             ...
             *
             ********************************************/
            case FT_WIFI_WNDRV_SET_REQ_ID:
            {
                FT_WiFi_Set_Req((FT_WIFI_WNDRV_SET_REQ *)ptrMsg->local_para_ptr, ptrMsg->peer_buff_ptr);
                ptrMsg->peer_buff_ptr=NULL; // let WNDRV free peer buffer
                break;
            }
            case FT_WIFI_WNDRV_QUERY_REQ_ID:
            {
                FT_WiFi_Query_Req((FT_WIFI_WNDRV_QUERY_REQ *)ptrMsg->local_para_ptr, ptrMsg->peer_buff_ptr);
                ptrMsg->peer_buff_ptr=NULL; // let WNDRV free peer buffer
                break;
            }
            case FT_WIFI_REQ_ID:
            {
                FT_WiFi_Operation((FT_WIFI_REQ *)ptrMsg->local_para_ptr, ptrMsg->peer_buff_ptr);
                break;
            }
            ...
            case FT_NFC_CMD_REQ_ID:
            {
                /* The FT_NFC_Operation function will return not supported state to PC side */
                FT_NFC_Operation(ptrMsg);
                break;
            }
            case FT_GPS_CMD_REQ_ID:
            {
                /* The FT_GPS_Operation function will return not supported state to PC side */
                FT_GPS_Operation(ptrMsg);
                break;
            }
            default:
            break;
    ...
    else if( (ptrMsg->src_mod_id == MOD_NVRAM) )
    {
       switch (ptrMsg->msg_id)
       {
           ...
           case MSG_ID_NVRAM_RESET_CNF:
           {
               ...
               ilm_ptr = FT_ALLOC_MSG( sizeof(ft_nvram_reset_cnf_struct_T) );
               /* if ptrMsg != NULL*/
           ...
           case MSG_ID_NVRAM_SET_LOCK_CNF:
           {
               ...
               ilm_ptr = FT_ALLOC_MSG( sizeof(ft_nvram_lock_cnf_struct_T) );
               /* if ptrMsg != NULL*/
...
/* under construction !*/
    ...
    /* Protocol changed in 11B HAL */
    else if( (ptrMsg->src_mod_id == MOD_MED) )
    {
        FT_L4Audio_MedConfirm(ptrMsg);
    }
...
/* under construction !*/
#endif // #if defined(__UMTS_RAT__) && defined(__MTK_UL1_FDD__)
#if defined(__WIFI_SUPPORT__)
    else if( (ptrMsg->src_mod_id == MOD_WNDRV) )
    {
        switch (ptrMsg->msg_id)
        {
        case MSG_ID_WNDRV_TEST_SET_CNF:
            FT_WiFi_Set_Cnf((wndrv_test_set_cnf_struct *)ptrMsg->local_para_ptr);
            break;
        case MSG_ID_WNDRV_TEST_QUERY_CNF:
            FT_WiFi_Query_Cnf((wndrv_test_query_cnf_struct *)ptrMsg->local_para_ptr, ptrMsg->peer_buff_ptr);
            ptrMsg->peer_buff_ptr=NULL; // let TST free peer buffer
            break;
        default:
            break;
        }
    ...
    /*********************************************
     *
     *   Dispatch TDMB CNF to TST and send back to PC
     *
     *********************************************/
#if defined(__TDMB_SUPPORT__)
    else if( MOD_TDMB == ptrMsg->src_mod_id)
    {
        FT_TDMB_SEND_CNF_BACK(ptrMsg);
    }
#endif  /* __TDMB_SUPPORT__*/

    /*********************************************
     *
     *   Dispatch CMMB CNF to TST and send back to PC
     *
     *********************************************/
#if defined(__CMMB_SUPPORT__)
    else if( MOD_CMMB == ptrMsg->src_mod_id)
    {
        FT_CMMB_SEND_CNF_BACK(ptrMsg);
    }
#endif  /* __CMMB_SUPPORT__*/
#if (!defined(__SMART_PHONE_MODEM__))
    /**************************************************
     * This message hanlder is used for wave streaming
     * and it's send by L1SP in DRVIVER_PS_SAP
     *************************************************/
    else if(MOD_L1SP == ptrMsg->src_mod_id && ptrMsg->sap_id != FT_TST_SAP)
    {
        ft_audio_play_wave_event_callback(ptrMsg);
    }
    /**************************************************
     * FT task self message handler
     * This message handler can be used in the following
     * purpose
     * 1. message handler for HISR handlers' requests
     * (currently used for Dual-Mic NR echo testing
     *************************************************/
    else if((MOD_L1SP == ptrMsg->src_mod_id || MOD_L1SPHISR == ptrMsg->src_mod_id) && ptrMsg->sap_id == FT_TST_SAP)
    {
        switch (ptrMsg->msg_id)
        {
            case MSG_ID_FT_IN_PROC_CALL_REQ:
            {
                ft_in_proc_call_handler(ptrMsg);
                break;
            }
        }
    }
    ...
    else if( (ptrMsg->src_mod_id == MOD_SIM) && (ft_gl_sim_cmd_type != FT_MISC_OP_CHECK_GEMINI_PLUS_SIM_INSERTED))
    {
       switch (ptrMsg->msg_id)
       {
           case MSG_ID_SIM_RESET_CNF:
           {
               ...
               misc_cnf.result.m_u1SIMInserted = msg_ptr->is_sim_inserted; // get SIM inserted status here.  1:insert  , 0: not insert
...
/* under construction !*/
    ...
    else if( MOD_ISP_HISR==ptrMsg->src_mod_id ) {
        ft_cct_proc_ae_awb();
    }
#endif // #if defined(ISP_SUPPORT)
    else if( MOD_DRV_HISR == ptrMsg->src_mod_id ) {
        ft_cct_usb_communication(ptrMsg);
    }
#if defined(__NFC_SUPPORT__)
    else if( MOD_NFC == ptrMsg->src_mod_id || MOD_NFC_APP == ptrMsg->src_mod_id )
    {
        FT_NFC_ConfirmHandler(ptrMsg);
    }
#endif // #if defined(__NFC_SUPPORT__)
#if defined(__GPS_SUPPORT__)
    else if( MOD_GPS == ptrMsg->src_mod_id)
    {
        FT_GPS_ConfirmHandler(ptrMsg);
    }
...
void FT_Task(task_entry_struct * task_entry_ptr)
{
    ...
    while ( 1 )
    {
        ...
        // if PC side META_DLL version is incorrect, power off target
        if(g_b_ft_assert_check_enable)
        {
            if(g_b_ver_Incorrect || g_u1_msg_recv_times >1)
            {
                if(g_b_ver_Incorrect)
                {
                    sprintf(g_FT_debug_buf, "[FT Task] Tool version check fail!(msg_times:%d)",g_u1_msg_recv_times);
                }
                else
                {
                    sprintf(g_FT_debug_buf, "[FT Task] Tool version check required! (msg_times:%d)",g_u1_msg_recv_times);
                }
                tst_sys_trace(g_FT_debug_buf);
                kal_sleep_task(50);
                if(g_b_ver_Incorrect){
                    ASSERT(META_Version_Check_Fail);
                }
                else{
                    ASSERT(META_Version_Check_Required);
                }
            }
            g_u1_msg_recv_times++;
        }
