#if defined(__IOT_CUSTOMIZE__)

#include "agps_adaptor.h"
#include "gps_adaptor.h"

#define GPSADAPTOR_TRACE(...) NULL
...
void gpsadaptor_agps_setting_get_profile_interface(gpsadaptor_agps_profile_struct* agps_profile){
    ...
    GPSADAPTOR_TRACE((TRACE_GROUP_2, GPSADAPOOR_AGPS_GET_AGPS_PROFILE_SETTING,
					profile.addr,
					profile.port));
	return;
}
...
static gpsadaptor_agps_result_enum gpsadaptor_agps_send_start_supl_req(kal_bool force_default_slp)
{
	...
	//kal_prompt_trace(MOD_GPSADAPTOR, "[AGPS_ADAPTOR]:1\n");
	if(gpsadaptor_agps_check_state_right() == KAL_FALSE){
	...
	#ifndef __IOT_CUSTOMIZE__ //Because of repeat
	...
	//kal_prompt_trace(MOD_GPSADAPTOR, "[AGPS_ADAPTOR]:4\n");
 ...
 #endif
	//kal_prompt_trace(MOD_GPSADAPTOR, "[AGPS_ADAPTOR]:5\n");
    ...
    gpsadaptor_agps_setting_get_up_setting_interface(&agps_up_setting);
	//kal_prompt_trace(MOD_GPSADAPTOR, "[AGPS_ADAPTOR]:2\n");
    ...
    GPSADAPTOR_TRACE((TRACE_GROUP_2, GPSADAPOOR_AGPS_GET_AGPS_CONFIG_SETTING,
					local_param_ptr->slp_addr,
					local_param_ptr->slp_port,
					local_param_ptr->use_tls));
...
void gpsadaptor_agps_p_callback(gpsadaptor_gps_parser_p_info_enum type)
{
    //GPSADAPTOR_TRACE(GPSADAPTOR_MEDIA_TRC_G2_APP, GPSADAPTOR_TRC_GPS_MGR_UART_P_CALLBACK, type);
...
void agps_adaptor_set_config(MMI_BOOL supl_en, 
                        ...
                        MMI_BOOL use_tls)
{
	kal_prompt_trace(MOD_GPSADAPTOR,"[agps adaptor]set config, SE[%d]SN[%s]SP[%d], use_tls%d", supl_en, supl_name, supl_port,use_tls);
...
#endif
// move all the callback to gps_main.c
#endif
