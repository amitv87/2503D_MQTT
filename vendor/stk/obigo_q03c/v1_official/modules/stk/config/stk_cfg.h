...
#ifdef CUSTOM_BROWSER_CACHE
#ifndef STK_CONFIG_CACHE
#define STK_CONFIG_CACHE
#endif
#endif
...
#ifdef WAP_XYSSL_SUPPORT
#define STK_CONFIG_TLS
#define STK_CONFIG_SSL
//#define STK_CONFIG_DIGEST_AUTH
#endif /* WAP_XYSSL_SUPPORT */ 
...
#if ((defined OBIGO_Q03C_BROWSER ) || (defined WPS_SUPPORT) || (defined __J2ME__))
#define STK_CONFIG_AUTH
#endif
...
/* If server stops responding or socket gets closed while downloading 
 * some content, default behavior is to abort the transaction and to notify
 * the application about error. For some requests, it is possible to retry
 * again with "Range" header for remaining content without user confirmation
 * using the following compile option. 
 * It is also possible to configure the number of retries in such cases using 
 * STK_CFG_MAX_AUTO_RETRIES macro definition. */
#define STK_CFG_AUTO_RESUME_DOWNLOAD

#ifdef STK_CFG_AUTO_RESUME_DOWNLOAD
#define STK_CFG_MAX_AUTO_RETRIES                  5
/* Time difference between socket close and retry.
 * It should be seconds*10 format.
 * Can be disabled if not required */
//#define STK_CFG_WAIT_BEFORE_AUTO_RESUME_TIME      50   /* (5*10) = 5 seconds */
#endif
...
#ifdef WAP_XYSSL_SUPPORT
#define STK_CFG_MAX_NUM_OF_SECURE_CONN 1
#else
