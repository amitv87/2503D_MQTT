...
__TEMPLATE
        ...
        if($tiny_sys eq 'TRUE')
        {
            $output_result .= &layout_tinysys($bb);
...
sub scatGen_verno
{
    return "v7.10 -> p7.14 -> p7.15" . " + scatFrame". scatFrame::scatFrame_verno() ;
    # p7.15 by Dongming,  20150515, put l1 obj into cached_extsram when zimage false
    # p7.14 by Yinli,  20140421, Reserve 12kB ram-space for SWLA when zimage using BEST_PERF policy
    # v7.11 - reserved for add WIFI_ROM on IOT FOTA project [11CW1411SP]
    # v7.10 by Yinli,  20140328, Unify cached dummy end calculation in GetBottomLayout() and layout15()
    # v7.09 by Yinli,  20140324, Adjust intsram distribution to reslove INTSRAM_CODE expired when TINY_SYSTEM is enabled
    # v7.08 - [ScatGenBL.pl]reserved for patch back IOT-fota [11BW1308MP] 
    # v7.07 by Yinli,  20140227, [scatGen.pl]Gen ketest related macro in ts_mem_config.h
    # v7.06 by Yinli,  20140226, Support Tiny System
    # v7.05 by Yinli,  20140128, Set EXT_BL max-size to 28kB when SECURE_SUPPORT is enabled on 32_32 segment for MT6260 and MT6261 platform
    # v7.04 by Yinli,  20140117, Support MT6261 family
    # v7.03 by Yinli,  20140117, [scatGen.pl]Sync custom_img_config.h with layout13
    # v7.02 by Yinli,  20140108, Fix ESD ROM patch side effect: BL size increases and BL overlap with ext_BL
    # v7.01 by Yinli,  20140103, Support BL scatter file re-org
    # v7.00 by Yinli,  20131217, remove RO-CODE in MBA region
...
sub GetBottomLayout
{
    ...
    my $lv_dcmp_code;
    my $nDCMP_evTotalSize = 0;
    my ($nCachedDummyEnd_BaseAddr, $nOffset, $temp_ref) = &GetDummyEnd($bb, $rs); # Calculate DCMP start addresses
...
#****************************************************************************
# subroutine:  layout_tinysys
# return:      tiny system layout
#****************************************************************************
sub layout_tinysys
{
    my $bb = shift;
    my ($TS_ROM1_SIZE, $TS_ROM2_SIZE) = (44*1024,45*1024);
### TINYSYS_ROM1 base and size
    my $ts_rom1_base = &config_query_rom_offset($bb);
    my $ts_rom1_size = $TS_ROM1_SIZE;
    my $ts_rom1_base_str = sprintf("0x%08x",$ts_rom1_base+&config_query_flash_base_address($bb));
    my $ts_rom1_size_str = sprintf("0x%08x",$ts_rom1_size);
### TINYSYS_ROM2 base and size
    my $ts_rom2_base = $ts_rom1_base + $ts_rom1_size;
    my $ts_rom2_size = $TS_ROM2_SIZE ; 
    my $ts_rom2_base_str = sprintf("0x%08x",$ts_rom2_base+&config_query_flash_base_address($bb));
    my $ts_rom2_size_str = sprintf("0x%08x",$ts_rom2_size);
### Set MAUI ROM start offset
#print "ts_rom2_base+ts_rom2_size = $ts_rom2_base+$ts_rom2_size\n";
    &config_add_rom_offset($ts_rom1_size+$ts_rom2_size);

    my $layout_tinysys = <<"__LAYOUT";
TS_ROM1 $ts_rom1_base_str $ts_rom1_size_str
{
}

TS_ROM2 $ts_rom2_base_str $ts_rom2_size_str
{
}

__LAYOUT
    return $layout_tinysys;
}
#****************************************************************************
...
#   +------------------+<-----  extsram end address
#   High    
    #share memory if tiny system is enabled
    my $tiny_sys_support = &$feature_query_function_ptr('tiny_sys');
    if(defined $tiny_sys_support and $tiny_sys_support eq 'TRUE')
    {
        ...
        $nOffset += hex($strCSCIInfoSize); 
        $RegionInfo{"TS_CSCIINFO"} = [ hex($strCSCIInfoSize), $nOffset ];
        
    	my $ts_res_size = hex($strTSMMDumpSize)+hex($strAPDspramBakSize)+hex($strAPIntsramBakSize)+hex($strTSLogSize)+hex($strTSResEmiSize);
    	$nOffset += $ts_res_size; 
        $RegionInfo{"TS_RESMEM"} = [ $ts_res_size, $nOffset ];
    }
...
TRACK_RW 0x103F0000
{
	...
	TRACK_RAM 0x00200000 0x00100000 
	{
		*track.lib  (+RW, +ZI )		 
	}
}  
TRACK_NVRAM 0x103F5000
{
	TRACK_NVRAM_GFH +0x0
	{
         *                             (TRACK_NVRAM_GFH, +First)
	}
	TRACK_NVRAM +0x0
	{
		*                   (TRACK_NVRAM_RODATA)
...
sub EXTSRAM_type_2
{
    ...
    my ($region_base_tx,$region_base_rx);
    $region_base_rx = &ev_extsram_base($bb) + $ram_size - $region_size_rx;
    $region_base_tx = $region_base_rx - $region_size_tx;
    $ev_string .= &DSPTXRX( $bb, "EXTSRAM", $region_base_tx, $region_base_rx,$region_size_tx, $region_size_rx);
...
sub EXTSRAM_type_6
{
    ...
    my ($region_size_tx, $region_size_rx) = &sysUtil::DSPTXRX_query_length($bb, $mode);
    my ($region_base_tx,$region_base_rx);
    $region_base_rx = 0x0 + $ram_size - $region_size_rx;
    $region_base_tx = $region_base_rx - $region_size_tx;
    $ev_string .= &DSPTXRX( $bb, "EXTSRAM", $region_base_tx, $region_base_rx,$region_size_tx, $region_size_rx);
...
sub EXTSRAM_type_7
{
    ...
    my($cached_dummy_end_start_address, $cached_dummy_end_start_address_val,$nOffset, $RegionInfo);
    ($cached_dummy_end_start_address_val, $nOffset, $RegionInfo_ref)= &GetDummyEnd($bb,$ram_size);
    $cached_dummy_end_start_address_val += $ram_base;
    print"cached_dummy_end_start_address_val=$cached_dummy_end_start_address_val\n";
    
    #DSP TX/RX
    my $mode = &$feature_query_function_ptr('MODE');
    my ($region_size_tx, $region_size_rx) = &sysUtil::DSPTXRX_query_length($bb, $mode);
    my ($region_base_tx, $region_base_rx,$ts_total_size);
    my $tiny_sys_support = &$feature_query_function_ptr('tiny_sys');
    if(defined $tiny_sys_support and $tiny_sys_support eq 'TRUE')
    {
        my ($strTSMMDumpSize,$strAPDspramBakSize,$strAPIntsramBakSize,$strTSLogSize,$strTSResEmiSize,$strCSCIInfoSize) = &sysUtil::GetTS_ResMemSize($bb);
        $ts_total_size = hex($strTSMMDumpSize)+hex($strAPDspramBakSize)+hex($strAPIntsramBakSize)+hex($strTSLogSize)+hex($strTSResEmiSize)+hex($strCSCIInfoSize);	
    }
	
    $region_base_rx = $ram_base + $ram_size - $ts_total_size - $region_size_rx;
    $region_base_tx = $region_base_rx - $region_size_tx;
    #DSP_SHAREMEM
    my ($dsp_sharemem_size, $mcu_c_dsp_c_len, $mcu_c_dsp_nc_len, $mcu_nc_dsp_c_len, $mcu_nc_dsp_nc_len) = &DSP_SHAREMEM_query_length($bb);
    ...
    $ev_string .= &app_feature_jblend( $ram_base+$ram_size-$RegionInfo->{"JBLEND"}[1] );
    $ev_string .= &EXTSRAM_PAGETABLE_EV($bb, $ram_base+$ram_size-$RegionInfo_ref->{"EXTSRAM_TMPPAGETABLE"}[1]);
    $ev_string .= &DSP_SHAREMEM_EV( $bb, "EXTSRAM", $ram_base+$ram_size-$RegionInfo_ref->{"SHAREMEM"}[1], $mcu_c_dsp_c_len, $mcu_c_dsp_nc_len, $mcu_nc_dsp_c_len, $mcu_nc_dsp_nc_len );
    $ev_string .= &DSPTXRX( $bb, "EXTSRAM", $region_base_tx, $region_base_rx,$region_size_tx, $region_size_rx);
    $ev_string .= &TS_RESERVE_MEM_EV($ram_base+$ram_size-$RegionInfo_ref->{"TS_RESMEM"}[1],$RegionInfo_ref->{"TS_RESMEM"}[0]);
    $ev_string .= &TS_CSCI_INFO_EV($ram_base+$ram_size-$RegionInfo_ref->{"TS_CSCIINFO"}[1],$RegionInfo_ref->{"TS_CSCIINFO"}[0]);
    $ev_string .= &shouldbe_emptysection();
    return $ev_string;
}
...
sub EXTSRAM_type_9
{
    ...
    my ($region_size_tx, $region_size_rx) = &sysUtil::DSPTXRX_query_length($bb, $mode);
    my ($region_base_tx,$region_base_rx);
    $region_base_rx = $extsram_base_value + $ram_size - $region_size_rx;
    $region_base_tx = $region_base_rx - $region_size_tx;
    $ev_string .= &DSPTXRX( $bb, "SECONDARY_EXTSRAM", $region_base_tx, $region_base_rx,$region_size_tx, $region_size_rx);
...
sub DSPTXRX
{
    my ($bb, $prefix_name, $region_base_tx, $region_base_rx,$region_size_tx, $region_size_rx) = @_;
    my $ev_addr_tx = sprintf("0x%08x", $region_base_tx);
    my $ev_addr_rx = sprintf("0x%08x", $region_base_rx);
...
{
    my ($base_addr) = @_;
    #for FOTA project, can not update while zimage open
    #so,while FOTA open, zimage must be closed.
    #and if Modem exist, L1 related obj need run on extsram for timing request.
    #only support 6261/2501/2502
    my $cached_extram_ro = "";
    my $zimage = &$feature_query_function_ptr('zimage_support');
    
    if (((!defined $zimage) or (defined $zimage and $zimage eq 'FALSE')) and
        ($g_bb eq "MT6261" or $g_bb eq "MT2501" or $g_bb eq "MT2502"))
    {

        $cached_extram_ro = <<"__TEMPLATE";
...
sub TS_RESERVE_MEM_EV
{
    my($ts_res_mem_base,$ts_res_mem_size) = @_;
    my $ts_res_mem_base_str = &CommonUtil::Dec2Hex($ts_res_mem_base);
    my $ts_res_mem_size_str = &CommonUtil::Dec2Hex($ts_res_mem_size);
    my $ts_support = &$feature_query_function_ptr('tiny_sys');
    my $ev_string;
    if(defined $ts_support and $ts_support eq 'TRUE')
    {
        $ev_string = &format_execution_view( "TS_RESERVE_MEM",
                                          $ts_res_mem_base_str,
                                          $ts_res_mem_size_str,
                                          [""]
                                        );                                      
...
sub TS_CSCI_INFO_EV
{
    my($ts_csci_info_base,$ts_csci_info_size) = @_;
    my $ts_csci_info_base_str = &CommonUtil::Dec2Hex($ts_csci_info_base);
    my $ts_csci_info_size_str = &CommonUtil::Dec2Hex($ts_csci_info_size);
    my $ts_support = &$feature_query_function_ptr('tiny_sys');
    my $ev_string;
    if(defined $ts_support and $ts_support eq 'TRUE')
    {
        $ev_string = &format_execution_view( "CSCI_DATA",
                                          $ts_csci_info_base_str,
                                          $ts_csci_info_size_str,
                                          ["*.* (csci_static_*)"]
                                        );   
    }
    return $ev_string; 
