...
# Constants
#****************************************************************************
my $CFGGEN_VERNO     = " V0.39";
                      #  v0.39 , support GPS bin download.
                      #  v0.38 , Support iotgen, which is similar with ximgen for IOT FOTA project.
                      #  v0.37 , - reserved for modify cfg filename to IMAGE.cfg as customer security 
                      #              concern on N project. More details, refer to NIKI_00022272
                      #  v0.36 , Support MT6261 family
                      #  v0.35 , - reserved for supporting CARD_DOWNLOAD on MT6260C(11BW1308MP)
...
if ($ACTION == 1)
{
    ...
    if ((defined &config_query_sv5($PLATFORM)) and (&config_query_sv5($PLATFORM) == 1))
    {
        ...
        #print CFG_FILE &gen_gps_region_setting();
...
if ($ACTION == 2)
{
    ### Only generate ini file for Card DL and IOT FOTA 
    if (((defined $MAKEFILE_OPTIONS{'card_download'}) and ($MAKEFILE_OPTIONS{'card_download'} ne 'NONE'))
         or((defined $MAKEFILE_OPTIONS{'funet_enable'}) and ($MAKEFILE_OPTIONS{'funet_enable'} ne 'NONE')))
    {
...
#****************************************************************************
# subroutine:  gen_gps_region_setting
# return:      GPS Region Setting
#****************************************************************************
sub gen_gps_region_setting
{
    my $gps_region_list;
    print "=============================dm\n";
    ...
    foreach (keys %bin_list)
    {
        if (/MT3333/)
        {         
            $gps_region_list .= "    - file: " . $_ . "\n";
            last;
        }
    }
    
    chomp $gps_region_list;
