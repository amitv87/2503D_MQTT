...
/* under construction !*/
#endif    
    {
        ...
        NVRAM_EF_CAT_TIMESTAMP_TOTAL,
        NVRAM_EF_CAT_TIMESTAMP_SIZE,
        NVRAM_NORMAL(NVRAM_EF_CAT_TIMESTAMP_DEFAULT),
        NVRAM_CATEGORY_INTERNAL,
        NVRAM_ATTR_MULTIPLE | NVRAM_ATTR_WRITEPROTECT,
        "CA00",
        VER(NVRAM_EF_CAT_TIMESTAMP_LID)
    },    
...
void nvram_gen_assign_entry(nvram_ltable_entry_struct* ldi)
{
    ...
    if (NVRAM_IS_CATEGORY_CALIBRAT(ldi->category))
    {
        ...
        ldi->attr &= ~NVRAM_ATTR_BACKUP_FAT;
    #else
        #if !defined(__LOW_COST_SUPPORT_COMMON__) && !defined(__SMART_PHONE_MODEM__)
        ldi->attr |= NVRAM_ATTR_MULTIPLE;
        #endif
        ldi->attr &= ~NVRAM_ATTR_BACKUP_RAW;
        ldi->attr &= ~NVRAM_ATTR_BACKUP_FAT;
        ...
        NVRAM_IS_CATEGORY_IMPORTANT_L4(ldi->category))
    {
        ...
        ldi->attr |= NVRAM_ATTR_BACKUP_RAW;
        ldi->attr &= ~ NVRAM_ATTR_BACKUP_FAT;
        nvram_gen_backup_size += (ldi->size + NVRAM_CHKSUM_SIZE) * ldi->total_records + FILE_PREFIX_LEN;
        nvram_gen_backup_total++;
    #endif
    ...
    #if defined(__MA_L1__) || defined(__WCDMA_RAT__) || defined(__UMTS_RAT__)
/* under construction !*/
    #else
        if (section_size * ldi->total_records > 2048)
    #endif
        {
...
static void nvram_gen_assign_cat_default_value(nvram_lid_enum position, nvram_cat_replace_setting* cat_item)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    nvram_ltable_entry_struct *ldi = nvram_gen_search_entry(position);

    printf("nvram_gen_assign_cat_default_value: %s/%s", ldi->str_LID, ldi->str_default_value);
...
void nvram_gen_construct_ltable(void)
{
    ...
    {
        nvram_gen_assign_cat_default_value(nvram_cat_replace_tbl[i].LID, &nvram_cat_replace_tbl[i]);
        i++;
    }
    #endif
