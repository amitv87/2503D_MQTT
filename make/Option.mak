...
ifdef TRACK_SENSOR_SUPPORT
  ifeq ($(strip $(TRACK_SENSOR_SUPPORT)),TRUE)
	...
	#test DA213
	#COMPOBJS         += custom\drv\track_sensor\DA213\Lib\carMotion.lib
	#CUS_REL_OBJ_LIST += custom\drv\track_sensor\DA213\Lib\carMotion.lib

       #delete these with __REMOVE_OBSOLETE_GSENSOR__ chengj
        #MC3410
       #COM_DEFS   += __MCUBE_MC3410__
       #COMMINCDIRS += custom\drv\track_sensor\MC3410

        #AFA750
       #COM_DEFS   += __MICRO_AFA750__
       #COMMINCDIRS += custom\drv\track_sensor\AFA750

        #MMA8452Q
       #COM_DEFS   += __FREESCALE_MMA8452Q__
       #COMMINCDIRS += custom\drv\track_sensor\MMA8452Q
...
ifneq ($(strip $(TRACK_CUST_GPS_SUPPORT)),NONE)
	...
	ifeq ($(strip $(TRACK_CUST_GPS_SUPPORT)),MT5631B)
		COM_DEFS	+= __GPS_MT5631B__
	endif
	ifeq ($(strip $(TRACK_CUST_GPS_SUPPORT)),G7020)
		COM_DEFS	+= __GPS_G7020__
	endif
	ifeq ($(strip $(TRACK_CUST_GPS_SUPPORT)),G3337)
		COM_DEFS	+= __GPS_G3337__
	endif
...
#------------------------------------------------------------ 	
	...
	#COM_DEFS +=__NETWORK_LICENSE_NOCALL__#关闭拨号功能
	
#------------------------------------------------------------
# function -2
#------------------------------------------------------------ 	
  	#是否是外贸版本  TRUE/FALSE
	#PROJECT_SUPPORT_TRACK_WM = TRUE  外贸控制放入各个项目OPTION里
	...
	#独立4K保存重要参数
	COM_DEFS += __IMPORTANT_PARAMETER_4K__
	...
	#快速开机  空壳 chengj
	#COM_DEFS +=__FAST_ON__

	#GPS秒定功能
	#COM_DEFS +=__SECOND_SET__   

	#屏蔽测试代码 节省代码空间  chengj
	COM_DEFS +=__SAVING_CODE_SPACE__  

	#屏蔽不再使用的gSensor chengj
	COM_DEFS +=__REMOVE_OBSOLETE_GSENSOR__  

	#删除日志写文件的功能 chengj
	COM_DEFS +=__REMOVE_LOG_TO_FILE__  

	#高低温回充功能宏，在其他文件打开 chengj
	#TEMP_RECHARGE_SUPPORT

ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT02E)

	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  

	#外部ADC 测量电瓶低电
	COM_DEFS	+= __EXT_VBAT_ADC__
	#开__GT02__的宏出的是GT02ES的软件
	COM_DEFS +=__GT02__
	COM_DEFS += __NO_ACC__
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT500)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT300)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
	#按键开机功能GT300/部分GT03D(新152适用)需要开这个宏--wangqi 20140122
	
	COM_DEFS += __KEY_POWER_ON__
	#多围栏
	COM_DEFS += __MORE_FENCE__
	#COM_DEFS += __NULL_SERVER__
	#短信电话走l4C接口GT530无RF功能
	COM_DEFS +=__L4C_API__
	COM_DEFS += __GFENCE__
	COM_DEFS += __NO_ACC__
	COM_DEFS += __GPS_INTERMITTENT_START__
	COM_DEFS +=__ALM_SOS__
endif

ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT300S)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
	...
	COM_DEFS += __GPS_INTERMITTENT_START__
	COM_DEFS +=__ALM_SOS__
	COM_DEFS+=__USE_SPI__
	COM_DEFS+=__ALM_BOOT__
	COM_DEFS+=__WIFI__

   #    COM_DEFS+=__BIG_CUR__
endif

ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT740)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifdef TRACK_PROJECT
	COM_DEFS+=__$(strip $(TRACK_PROJECT))__
endif

ifdef TRACK_PROJECT_NAME
	COM_DEFS+=__$(strip $(TRACK_PROJECT_NAME))__
endif
	
endif
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT420D)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = TRUE  
	...
	COM_DEFS+=__AMS_LOG__
      ...
      #中科微星历
      COM_DEFS +=__ZKE_EPO__

      #ATGM(中科微)固件在线升级功能	
	COM_DEFS +=__ATGM_UPDATE__

	#MQTT协议
	COM_DEFS    +=__MQTT__
	COM_DEFS+=__JSON__
	COM_DEFS    +=__DASOUCHE__
	
endif

ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT03F)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  

	#按键开机功能的GT03D(按键接152和模块)模块使用中断来检测按键--wangqi 20140213
	#出GT03D软件要开启此宏;不改变开机方法;开机由152实现
	COM_DEFS += __KEY_POWER_ON__
	COM_DEFS += __NO_ACC__
	#COM_DEFS += __NO_MIC__
	COM_DEFS += __GPS_INTERMITTENT_START__
	...
	#关机充电时到3.6000时自动开机
	#COM_DEFS += __CHR_POWERON__
	#出SOS版本软件时要开启__CHR_POWERON__出按键开机版软件时要关闭
	
	COM_DEFS += __KEY_EINT__
	#注意 __KEY_EINT__ 出按键开机版软件时要开启，出SOS版软件时要关闭
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),BD300)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),ET210)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
	...
	COM_DEFS +=__SERVER_TUQIANG__
	#COM_DEFS +=__SERVER_GPSLUCKLY__
	#COM_DEFS +=__SERVER_GTSX__#北京广泰顺心
	#短信电话走l4C接口GT530无RF功能
	COM_DEFS +=__L4C_API__
	COM_DEFS +=__FENCE__
endif

ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),ET130)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
	...
	COM_DEFS +=__KEY_EINT__
	COM_DEFS +=__UART1_SOS__
	COM_DEFS +=__JIMISHARE__
	#短信电话走l4C接口GT530无RF功能
	COM_DEFS +=__L4C_API__
	COM_DEFS +=__FENCE__
	COM_DEFS +=__ALM_SOS__
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GW100)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT530)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE
	...
	COM_DEFS +=__EXT_VBAT_ADC__
	COM_DEFS +=__SERVER_TUQIANG__
	COM_DEFS +=__GT500__#__GT06D__  #TEST
	COM_DEFS +=__OBD__
	COM_DEFS +=__KEY_EINT__
	COM_DEFS +=__FENCE__
	#不用联网
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT310)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
#JM66牦牛
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),JM66)	
#是否是外贸版本  TRUE/FALSE	
PROJECT_SUPPORT_TRACK_WM = FALSE  	
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),V20)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT800)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
#***************MT200  NT51的分界线**********
	...
	#with mcu
	COM_DEFS	+= __CUST_ACC_BY_MCU__
#**********************************

endif


ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT02F)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),NT22)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),ET310)
  ...
  ifeq ($(strip $(PROJECT_SUPPORT_TRACK_WM)),TRUE)
	#外贸版，旧协议
	COM_DEFS	+= __WM_OLD06__
	COM_DEFS	+= __ACC_STAUTS__
  endif	
	#COM_DEFS	+= __SERVER_COOACCESS__
	COM_DEFS	+= __SERVER_TUQIANG__
	#COM_DEFS	+= __SERVER_GPSLUCKLY__
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),ET320)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
# 曾用名 NT33 GV22 GV25
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),NT33)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
ifeq ($(strip $(PROJECT_SUPPORT_TRACK_WM)),TRUE)
	#hw v2.0  外贸版带ACC
	COM_DEFS	+= __FUN_WITH_ACC__
	
	#RTC protocol
	COM_DEFS	+= __RTC_OLD_PROTOCOL__

else
	#hw v1.0  国内版无ACC线
	COM_DEFS	+= __NO_ACC__
	#Virtual ACC 开GPS为ON，5分钟无震动为0FF
	COM_DEFS	+= __VIRTUAL_ACC__
...
################################################################################################
	...
	#定制版软件
	#迪迪网络-客制服务器的中文版，加新功能
	#COM_DEFS	+= __CUSTOM_DDWL__

	#域名解析失败使用上次的IP
	COM_DEFS 	+=__DNS_FAIL_USE_LAST_IP__

endif

# 曾用名 GT370 GT370A NT97
...
# 曾用名 GT380 NT98 GT840 NT34
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),GT380)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
	...
	#作为测试工具，配合验证RS485通信模块
	#COM_DEFS	+= FOR_RS485_TEST_TOOL

	#温湿度传感器 Temperature and humidity sensor
	COM_DEFS	+= __CUST_TEMP_HUMI_SENSOR__
	
endif

ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),ET350)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
...
#GT810 NT37
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),NT37)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = TRUE  
	...
	#油量数据上传 占用串口1，测试在开机10秒发指令
       COM_DEFS +=__OIL_DETECTION__

       #Multiple output
	COM_DEFS +=__MULTIPLE_OUTPUT__
	       
endif

#JM81
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),JM81)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
	...
	#支持新超长待机单片机协议	
	COM_DEFS +=__MCU_LONG_STANDBY_PROTOCOL__
...
#NF2310 HVT001
ifeq ($(strip $(PROJECT_SUPPORT_TRACK)),HVT001)
	#是否是外贸版本  TRUE/FALSE
	PROJECT_SUPPORT_TRACK_WM = FALSE  
