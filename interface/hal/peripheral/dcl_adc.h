...
#define ADC_CMDS \
   ...
   ADC_CMD_SET_BYPASS_ZCV,	\
   ADC_CMD_GET_ZCV,	\
   ADC_CMD_SET_IGNORE_BY_TS,	\
...
typedef struct
{
   ...
   DCL_UINT32 u4Sapid;
   module_type u1OwnerId;				// Indicate the module to for ADC driver to notify the result
...
//ADC_CMD_GET_CHANNEL
typedef enum {
#if defined(__MT200__) || defined (__ET310__) ||defined (__ET320__)||defined (__NT22__)
   DCL_VISENSE_ADC_CHANNEL=0,
   DCL_VBAT_ADC_CHANNEL,
#else
...
/* For ADC_CMD_SET_BYPASS_ZCV command. */
typedef struct
{
   DCL_UINT8      bBypassZcv;
} ADC_CTRL_SET_BYPASS_ZCV_T;

/* For ADC_CMD_SET_IGNORE_BY_TS command. */
typedef struct
{
   DCL_UINT8      ignore_by_ts;
} ADC_CTRL_SET_IGNORE_BY_TS_T;

/* For ADC_CMD_GET_ZCV command. */
typedef struct
{
   DCL_UINT32   u4Volt;
} ADC_CTRL_GET_ZCV_T;
...
/* For ADC_CMD_GET_IMM_DATA command. */
typedef struct
{
   DCL_UINT8 u1Channel;  //measure physical channel	
   DCL_DOUBLE   d8AdcValue;
...
/* For ADC_CMD_SEND_START command. */
typedef struct
{
   module_type u1OwnerId;				/* Indicate the module to for ADC driver to notify the result */
   DCL_UINT32     u4Handle;
...
#define ADC_CTRLS \
   ...
   ADC_CTRL_SEND_START_T rADCSendStart; \
   ADC_CTRL_SEND_STOP_T rADCSendStop; \
   ADC_CTRL_SET_BYPASS_ZCV_T rADCSetBypassZcv; \
   ADC_CTRL_GET_ZCV_T rADCGetZcv; \
   ADC_CTRL_SET_IGNORE_BY_TS_T rADCSetIgnoreByTS; \
   DCL_MULTI_CHANNEL_PARA_T rMultiChannelPara; \
   ADC_CTRL_GET_IMM_DATA_T rADCGetImmData;
