 ...
 *                 is not supported.
 */
void xyssl_rsa_init( rsa_context *ctx,
               int padding,
               int hash_id,
               int (*f_rng)(void *),
               void *p_rng );
 ...
 *
 * \note           xyssl_rsa_init() must be called beforehand to setup
 *                 the RSA context (especially f_rng and p_rng).
 *
 * \return         0 if successful, or an XYSSL_ERR_RSA_XXX error code
 */
int xyssl_rsa_gen_key(void *ssl, rsa_context *ctx, int nbits, int exponent );
 ...
 * \return         0 if successful, or an XYSSL_ERR_RSA_XXX error code
 */
int xyssl_rsa_check_pubkey( rsa_context *ctx );
 ...
 * \return         0 if successful, or an XYSSL_ERR_RSA_XXX error code
 */
int xyssl_rsa_check_privkey(void *ssl, rsa_context *ctx );
 ...
 */
int rsa_public(void *ssl, rsa_context *ctx,
                unsigned char *input,
 ...
 *                 enough (eg. 128 bytes if RSA-1024 is used).
 */
int rsa_private(void *ssl, rsa_context *ctx,
                 unsigned char *input,
                 unsigned char *output );
 ...
 */
int xyssl_rsa_pkcs1_encrypt(void *ssl, rsa_context *ctx,
                       int mode, int  ilen,
                       unsigned char *input,
                       unsigned char *output );
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int xyssl_rsa_pkcs1_decrypt(void *ssl, rsa_context *ctx,
                       int mode, int *olen,
                       unsigned char *input,
                       unsigned char *output );
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int xyssl_rsa_pkcs1_sign(void *ssl, rsa_context *ctx,
                    int mode,
                    int hash_id,
                    int hashlen,
                    unsigned char *hash,
 ...
 *                 of ctx->N (eg. 128 bytes if RSA-1024 is used).
 */
int xyssl_rsa_pkcs1_verify(void *ssl, rsa_context *ctx,
                      int mode,
                      int hash_id,
                      int hashlen,
                      unsigned char *hash,
 ...
 */
void rsa_free(void *ssl, rsa_context *ctx );
 ...
 */
int rsa_self_test(void *ssl, int verbose );
