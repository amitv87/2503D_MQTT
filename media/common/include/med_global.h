...
#define __MED_BT_HF_MOD__
#endif
#if defined(__BT_A2DP_PROFILE__)
#define __MED_BT_A2DP_MOD__
#endif 

#if defined(__MED_BT_HFP_MOD__) || defined(__MED_BT_A2DP_MOD__) || defined(__MED_BT_HF_MOD__)
...
// separater  melody with mma because of build error 
// if no define melody mma  will also not link ,so it will not influence the size
...
/* Multiple player control, not used in neptune project */
	...
	#if defined( __AUDIO_ULTRA_SLIM__) && (!defined(__J2ME__))
		#define __MED_SLIM_MMA__
	#endif
	#if defined(__J2ME__)
...
#if !defined(FMT_NOT_PRESENT) || defined(__AUDIO_AT_CMD__)
...
//#if defined(__MEGAPHONE_SUPPORT__) // if we want open megaphone ,we must set this macro
#define __MED_MEGA_PHONE__  
//#endif
#if defined (__POC__) || defined (STREAM_SUPPORT) || defined (__VOIP__) || defined (__STREAM_SERVER_SUPPORT__)
...
/* Auto build cache (10A afterward) */
    ...
    #if defined(LOW_COST_SUPPORT)
        #define AUD_MED_BUILD_CACHE_BUF_SIZE (8960)
    #else
...
/* Will be concurrently used with audio codec */
    ...
    #define AUD_DRV_BT_EXT_MEM_SIZE (16*1024) /* SCO buffer: 16384 bytes */
#elif defined(__BT_A2DP_PROFILE__)
    /* A2DP buffer: 16384 bytes  + 26208 bytes working in driver (original static 26KROM)*/
    #define AUD_DRV_BT_EXT_MEM_SIZE (16*1024 + 26368) 
#else
    #define AUD_DRV_BT_EXT_MEM_SIZE (0)
#endif

#if defined(__MCU_DTMF_SUPPORT__) && !defined( MED_MODEM )
    #define AUD_DRV_SW_DTMF_EXT_MEM_SIZE (11264) /*SW DTMF Working and Buffer memory : 8424 bytes*/
#else
...
/* APM */
   ...
   #define AUD_DRV_APM_LEVEL2_MEM_SIZE  (0)
    ...
    #define AUD_DRV_A2DP_SINK_SBC_PAR_COMP_MEM_SIZE (1024 * (3 + 8)) // Parser  Internal Memory (2468 --> 1024 x 3) + Parser  Output Buffer (4096 x 2 --> 1024 x 8)
    #define AUD_DRV_A2DP_SINK_SBC_DEC_COMP_MEM_SIZE (1024 * (5 + 8)) // Decoder Internal Memory (5108 --> 1024 x 5) + Decoder Output Buffer (4096 x 2 --> 1024 x 8)
    #define AUD_DRV_A2DP_SINK_SBC_DEC_MEM_SIZE    (AUD_DRV_A2DP_SINK_SBC_PAR_COMP_MEM_SIZE + AUD_DRV_A2DP_SINK_SBC_DEC_COMP_MEM_SIZE)
#else
    #define AUD_DRV_A2DP_SINK_SBC_DEC_MEM_SIZE    (0)
#endif

#if defined(__BT_SERIAL_PLAYBACK_AUDIO_SYNC__)
    #define AUD_DRV_BT_AUD_SYNC_MEM_SIZE    (1024 * (1 + 8)) // Internal Memory (388 --> 1024 x 1) + Output Buffer (4096 x 2 --> 1024 x 8)
#else
    #define AUD_DRV_BT_AUD_SYNC_MEM_SIZE    (0)
#endif
   ...
   #define AUD_DRV_VAD_EXT_MEM_SIZE (16*1024)
#else
   #define AUD_DRV_VAD_EXT_MEM_SIZE (0)
#endif
/******************************************************
...
#define AUD_WORKING_BUFFER_KARAOKE_REC (83*1024)
#else
#define AUD_WORKING_BUFFER_KARAOKE_REC (0)
#endif
...
#define AUD_WORKING_BUFFER_SBC_DEC      (AUD_DRV_A2DP_SINK_SBC_DEC_MEM_SIZE)
...
/* Audio memory use cases:              [Overhead]        + [BT]                    + [BT AUD SYNC]                + [SW DTMF]                    + [DSP Decompression and Audio Post-Process]                        + [Working buffer]             + [MPL Object buffer]      + [FS buffer]           + [Build Cache buffer]          + [Ring Buffer] */
#define MAX_AUD_MEM_SIZE_SPEECH         (AUD_MED_OVERHEAD +                       0 +                            0 +                            0 + MAXOF(AUD_DRV_DSP_NR_EXT_MEM_SIZE, AUD_DRV_DSP_EXT_MEM_SIZE))        /* Speech */
#define MAX_AUD_MEM_SIZE_VRSD           (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 +                            0 +                                                            0      + AUD_WORKING_BUFFER_VRSD)  /* VRSD */
#define MAX_AUD_MEM_SIZE_VRSI           (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 +                            0 +                                                            0      + AUD_WORKING_BUFFER_VRSI)  /* VRSI */
#define MAX_AUD_MEM_SIZE_TTS            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 +                            0 + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,                           0)      + AUD_WORKING_BUFFER_TTS)   /* TTS */
#define MAX_AUD_MEM_SIZE_CTM            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 +                            0 + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,                           0)      + AUD_WORKING_BUFFER_CTM)   /* CTM */
#define MAX_AUD_MEM_SIZE_BKSND          (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 +                            0 + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,                           0)      + AUD_WORKING_BUFFER_BGSND) /* Background sound */   
#define MAX_AUD_MEM_SIZE_MMA            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_WT_EXT_MEM_SIZE, AUD_DRV_APM_MIDI_EXT_MEM_SIZE) + AUD_WORKING_BUFFER_MMA       + AUD_MED_MPL_OBJ_BUF_SIZE) /* Multiple player */
#define MAX_AUD_MEM_SIZE_WAV            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_WAV       + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE + AUD_MED_BUILD_CACHE_BUF_SIZE + MED_RING_BUFFER_SIZE + AUD_WORKING_BUFFER_KARAOKE_REC)  /* WAV */
#define MAX_AUD_MEM_SIZE_MP3            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE + AUD_DRV_BT_AUD_SYNC_MEM_SIZE + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE_MP3,AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_MP3       + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE + AUD_MED_BUILD_CACHE_BUF_SIZE + MED_RING_BUFFER_SIZE + AUD_WORKING_BUFFER_KARAOKE_REC)  /* MP3 */
#define MAX_AUD_MEM_SIZE_MP4            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_MP4       + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE +                            0 + MED_MP4_RING_BUFFER_SIZE) /* MP4 */
#define MAX_AUD_MEM_SIZE_AAC            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE + AUD_DRV_BT_AUD_SYNC_MEM_SIZE + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_AAC       + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE + AUD_MED_BUILD_CACHE_BUF_SIZE + MED_RING_BUFFER_SIZE)  /* AAC */
#define MAX_AUD_MEM_SIZE_AMR            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_AMR       + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE + AUD_MED_BUILD_CACHE_BUF_SIZE + MED_RING_BUFFER_SIZE + AUD_DRV_VAD_EXT_MEM_SIZE)  /* AMR */
#define MAX_AUD_MEM_SIZE_AWB            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_AWB       + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE + AUD_MED_BUILD_CACHE_BUF_SIZE + MED_RING_BUFFER_SIZE)  /* AWB */
#define MAX_AUD_MEM_SIZE_APE            (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_APE       + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE +                            0 + MED_RING_BUFFER_SIZE)  /* APE */
#define MAX_AUD_MEM_SIZE_VORBIS         (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_VORBIS    + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE +                            0 + MED_RING_BUFFER_SIZE)  /* Vorbis */
#define MAX_AUD_MEM_SIZE_FLAC           (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE + AUD_DRV_BT_AUD_SYNC_MEM_SIZE + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_FLAC      + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE +                            0 + MED_RING_BUFFER_SIZE + AUD_WORKING_BUFFER_KARAOKE_REC)  /* Flac */
#define MAX_AUD_MEM_SIZE_SBC_DEC        (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE + AUD_DRV_BT_AUD_SYNC_MEM_SIZE + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_SBC_DEC   + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE +                            0 + MED_RING_BUFFER_SIZE + AUD_WORKING_BUFFER_KARAOKE_REC)  /* SBC Dec */
#define MAX_AUD_MEM_SIZE_I2S_REC        (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_EXT_MEM_SIZE,    AUD_DRV_APM_EXT_MEM_SIZE)      + AUD_WORKING_BUFFER_I2S_REC   + AUD_MED_MPL_OBJ_BUF_SIZE +                     0 +                            0 + MED_RING_BUFFER_SIZE)  /* I2S record */
#define MAX_AUD_MEM_SIZE_AMR_REC        (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_NR_EXT_MEM_SIZE,                        0)      + AUD_WORKING_BUFFER_AMR_REC   + AUD_MED_MPL_OBJ_BUF_SIZE +                     0 +                            0 + MED_RING_BUFFER_SIZE + AUD_DRV_VAD_EXT_MEM_SIZE)  /* AMR record */
#define MAX_AUD_MEM_SIZE_VORBIS_REC     (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_NR_EXT_MEM_SIZE,                        0)      + AUD_WORKING_BUFFER_VORBIS_REC+ AUD_MED_MPL_OBJ_BUF_SIZE +                     0 +                            0 + MED_RING_BUFFER_SIZE)  /* VORBIS record */
#ifdef MIDFILE_PLAY_SUPPORT
#define MAX_AUD_MEM_SIZE_MIDI_FILE      (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_WT_EXT_MEM_SIZE, AUD_DRV_APM_MIDI_EXT_MEM_SIZE) + AUD_WORKING_BUFFER_MIDI_FILE + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_MED_PROC_BUF_SIZE + AUD_WORKING_BUFFER_KARAOKE_REC) /* MIDI file playback */
#define MAX_AUD_MEM_SIZE_IMY_DATA       (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_WT_EXT_MEM_SIZE, AUD_DRV_APM_MIDI_EXT_MEM_SIZE) + AUD_WORKING_BUFFER_IMY_DATA  + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_WORKING_BUFFER_KARAOKE_REC) /* IMY data playback */
...
#define MAX_AUD_MEM_SIZE_MIDI_DATA      (AUD_MED_OVERHEAD + AUD_DRV_BT_EXT_MEM_SIZE +                            0 + AUD_DRV_SW_DTMF_EXT_MEM_SIZE + MAXOF(AUD_DRV_DSP_WT_EXT_MEM_SIZE, AUD_DRV_APM_MIDI_EXT_MEM_SIZE) + AUD_WORKING_BUFFER_MIDI_DATA + AUD_MED_MPL_OBJ_BUF_SIZE + AUD_WORKING_BUFFER_KARAOKE_REC + AUD_DRV_VAD_EXT_MEM_SIZE) /* MIDI/IMY data playback */
#endif
 ...
 **************************************************************************************************************/
    ...
    #define MAX_MED_CAM_CAP_LCD_EXT_MEM_SIZE_DISABLE_JAIA        (MIN_CAM_CAPTURE_LCD_ISP_BUF_SIZE + MAX_CAM_CAPTURE_LCD_ISP_WORK_BUF_SIZE + CAM_GDI_BLTDB_BUFFER_SIZE + MAX_CAM_CAPTURE_THUMBNAIL_BUF_SIZE)
...
#define __VAD_TRACE_ON__
#endif
#ifdef __LINE_IN_SUPPORT__
#define __LINEIN_TRACE_ON__
#endif

#ifdef __KARAOKE_SUPPORT__
#define __KARAOKE_TRACE_ON__
#endif
...
#ifdef __VAD_TRACE_ON__
#define AUD_VAD_FUNC_ENTRY1(F, x)    kal_trace(TRACE_GROUP_1, F, x);
#define AUD_VAD_FUNC_ENTRY2(F, x,y)    kal_trace(TRACE_GROUP_1, F, x,y);
#define AUD_VAD_FUNC_ENTRY3(F, x,y,z)    kal_trace(TRACE_GROUP_1, F, x,y,z);
#else
#define AUD_VAD_FUNC_ENTRY1(F, x)    
#define AUD_VAD_FUNC_ENTRY2(F, x,y)   
#define AUD_VAD_FUNC_ENTRY2(F, x,y,z)
#endif

#ifdef __LINEIN_TRACE_ON__
#define AUD_LINEIN_FUNC_ENTRY1(F, x)    kal_trace(TRACE_GROUP_1, F, x);
#define AUD_LINEIN_FUNC_ENTRY2(F, x,y)    kal_trace(TRACE_GROUP_1, F, x,y);
#else
#define AUD_LINEIN_FUNC_ENTRY1(F, x)    
#define AUD_LINEIN_FUNC_ENTRY2(F, x)   
#endif

#ifdef __KARAOKE_TRACE_ON__
#ifdef __KARAOKE_NEED_KEEP_SRC_PATH__
#define AUD_KARAOKE_FUNC_ENTRY(F)    kal_trace(TRACE_GROUP_1, F);
#endif /*__KARAOKE_NEED_KEEP_SRC_PATH__*/

#define AUD_KARAOKE_FUNC_ENTRY1(F, x)    kal_trace(TRACE_GROUP_1, F, x);
#define AUD_KARAOKE_FUNC_ENTRY2(F, x,y)    kal_trace(TRACE_GROUP_1, F, x,y);
#else
#define AUD_KARAOKE_FUNC_ENTRY1(F, x)    
#define AUD_KARAOKE_FUNC_ENTRY2(F, x)   
#endif
