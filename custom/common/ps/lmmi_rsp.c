...
void l4c_cc_exe_ata_lrsp(l4c_result_struct result, kal_uint8 call_mode){
    OTA_LOGD(L_CMD, L_V5, "======flag:%d,cause:%d,mode:%d",result.flag , result.cause, call_mode);
#ifdef __L4C_API__
    OTA_track_l4c_cc_exe_ata_lrsp(result.flag , result.cause, call_mode);
#endif /* __L4C_API__ */
}
...
void l4c_cc_exe_call_setup_lrsp(l4c_result_struct result,
                                ...
                                kal_uint8 diag){
    OTA_LOGD(L_CMD, L_V5, "======%d,cause:%d,call_id:%d,diag:%d",result.flag, result.cause, call_id, diag);
#ifdef __L4C_API__
    OTA_track_l4c_cc_exe_call_setup_lrsp(result.flag , result.cause, call_id);
#endif /* __L4C_API__ */
} //MAUI_02526481
...
{
    #ifdef __BCM_PBAP_SERVER__
    l4_phb_get_entry_by_index_rsp_struct *rsp = construct_local_para(sizeof(l4_phb_get_entry_by_index_rsp_struct),TD_RESET);
    
    (rsp->result).cause = result.cause;
    (rsp->result).flag = result.flag;
    rsp->no_list = no_list;
    rsp->phb_result = phb_result;
    kal_mem_cpy(rsp->list,list,sizeof(l4c_phb_entry_struct)*PHB_MAX_PHB_ENTRIES);

    l4c_send_msg_to_bcm(MSG_ID_MMI_PHB_GET_ENTRY_BY_INDEX_RSP,rsp);
    #endif
}
