...
static void track_l4c_send_msg_track_mmi(msg_type msgtype, void * msg, void * peer_msg)
{
    ilm_struct *ilm_ptr;
#ifdef __L4C_API__
...
void l4c_cc_call_ring_lind(kal_uint8 call_id,
    ...
    OTA_LOGD(L_CMD, L_V5, "======%s,sub:%s,red:%s, type:%d,signal:%d,%d,cause:%d,%d",
         num->number, sub_addr->number, redirect_num->number, call_type, signal_value_present, signal_value, no_cli_cause_present, no_cli_cause);

    //MSG_ID_MMI_CC_CALL_RING_IND
#if defined( __L4C_API__)
    OTA_track_l4c_sendmsg_call_ring_ind(num, call_type, call_id);
#endif /* __L4C_API__ */
								  }
...
void l4c_cc_cpi_lind(kal_uint8 call_id,
		     ...
		     kal_uint8 src_id,
                     kal_uint8 call_type){
kal_prompt_trace(2, "l4c_cc_cpi_lind call_id:%d,msg_type:%d,dir:%d,call_mode:%d,number->number:%d,disc_cause:%d", call_id, msg_type, dir, call_mode, number->number, disc_cause);
}
...
void l4c_cc_data_call_req_lind(kal_uint8 source_id,
                               ...
                               kal_uint8 call_type,
                               l4c_number_struct number){
kal_prompt_trace(2, "l4c_cc_data_call_req_lind source_id:%d, %d %d,%d,%s", source_id, opcode, call_type,number.number);
...
{
#ifdef __L4C_API__
    OTA_track_l4c_sendmsg_call_disc_release_lind(0, call_id, cause, call_type, diag);
#endif /* __L4C_API__ */
} //MAUI_02526481
#endif
...
#endif
#ifdef __L4C_API__
    OTA_track_l4c_sendmsg_call_disc_release_lind(1, call_id, cause, 0, diag);
#endif /* __L4C_API__ */
}
...
#endif
    OTA_LOGD(L_CMD, L_V5, "======%s,%d,%d", num->number, call_type, call_id);
#ifdef __L4C_API__
    OTA_track_l4c_cc_call_connect_lind(num, call_type, call_id);
#endif /* __L4C_API__ */
}
#ifndef  __EMPTY_CALLBACK__
...
{
    mmi_nw_attach_ind_struct *myMsgPtr = NULL;
    OTA_LOGD(L_CMD, L_V6, "status:%d,plmn:%d,gsm:%d,gprs%d,%d,rat:%d,%d,%d,%d,%d,%d,%d,%d,%d", 
        status, *plmn, gsm_state, gprs_state, gprs_status, rat, cell_support_egprs,
        *lac, rac, cell_id, cause, data_speed_support, is_on_hplmn, domain);
#ifdef __L4C_API__    
    myMsgPtr = (mmi_nw_attach_ind_struct*) construct_local_para(sizeof(mmi_nw_attach_ind_struct), TD_CTRL);
    myMsgPtr->status = status;
    memcpy(myMsgPtr->plmn, plmn, MAX_PLMN_LEN);
    memcpy(myMsgPtr->lac, lac, 2);
    myMsgPtr->gsm_state = gsm_state;
    myMsgPtr->gprs_state = gprs_state;
    myMsgPtr->gprs_status = gprs_status;
    myMsgPtr->rat = rat;
    myMsgPtr->cell_support_egprs = cell_support_egprs;
    myMsgPtr->rac = rac;
    myMsgPtr->cell_id = cell_id;
    myMsgPtr->cause = cause;
    myMsgPtr->data_speed_support = data_speed_support;

    track_l4c_send_msg_track_mmi(MSG_ID_MMI_NW_ATTACH_IND, myMsgPtr, NULL);
#endif /* __L4C_API__ */
} 
...
#ifdef __L4C_API__
    static kal_int32 rssi = 0;
    kal_int32 cur_rssi = 0;
    
    cur_rssi = custom_signal_strength_raw_to_csq_level();
    
    OTA_LOGD(L_CMD, L_V9, "%d rat:%d,rssi:%d,RSCP:%d,EcNo:%d,ber:%d,band:%d",
         cur_rssi,rat, rssi_in_qdbm, RSCP_in_qdbm, EcN0_in_qdbm, ber, current_band);

    if(cur_rssi != rssi && rat == 1)
    {
        OTA_LOGD(L_CON, L_V9, "RSSI:%d,%d,%d", cur_rssi, ber, current_band);
        rssi = cur_rssi;
        OTA_track_cust_get_rssi(cur_rssi);
		                                ...
		                                kal_uint8 *plmn)
{
	...
	updateTime.nYear = srv_nitz_convert_time(nw_time_zone_time.year) + YEARFORMATE;
	updateTime.nMonth = srv_nitz_convert_time(nw_time_zone_time.month);
	updateTime.nDay = srv_nitz_convert_time(nw_time_zone_time.day);
	updateTime.nHour = srv_nitz_convert_time(nw_time_zone_time.hour);
	updateTime.nMin = srv_nitz_convert_time(nw_time_zone_time.min);
	updateTime.nSec = srv_nitz_convert_time(nw_time_zone_time.sec);
	t_sec = applib_dt_mytime_2_utc_sec(&updateTime, KAL_FALSE); 
	gps_time_sync_nitz_sync(t_sec);
#endif
...
#ifdef __BCM_PBAP_SERVER__
extern void l4c_send_msg_to_bcm(msg_type msg_name, void *param_ptr);
#endif

void l4c_phb_startup_read_lind(l4c_phb_entries_struct *phb_entries, kal_uint8 access_id)
{
    #ifdef __BCM_PBAP_SERVER__
    l4_phb_startup_read_ind_struct *ind = construct_local_para(sizeof(l4_phb_startup_read_ind_struct),TD_RESET);

    ind->access_id = access_id;
    kal_mem_cpy(ind->phb_entries,phb_entries,sizeof(l4c_phb_entries_struct));
    l4c_send_msg_to_bcm(MSG_ID_MMI_PHB_STARTUP_READ_IND,ind);
    #endif    
}

void l4c_phb_startup_begin_lind(l4c_phb_desc_struct desc[PHB_TYPE_TOTAL])
{
    #ifdef __BCM_PBAP_SERVER__
    l4c_send_msg_to_bcm(MSG_ID_MMI_PHB_STARTUP_BEGIN_IND,NULL);
    #endif
...
{
    #ifdef __BCM_PBAP_SERVER__
    l4c_write_phb_req_ind_struct *ind = construct_local_para(sizeof(l4c_write_phb_req_ind_struct),TD_RESET);

    ind->src_id = src_id;
    kal_mem_cpy(&(ind->entry),&entry,sizeof(l4c_phb_entry_struct));
    
    l4c_send_msg_to_bcm(MSG_ID_MMI_L4C_WRITE_PHB_REQ_IND,ind);
    #endif
}
...
{
    #ifdef __BCM_PBAP_SERVER__
    l4c_delete_phb_req_ind_struct *ind = construct_local_para(sizeof(l4c_delete_phb_req_ind_struct),TD_RESET);

    ind->src_id = src_id;
    ind->type = type;
    ind->storage = storage;
    ind->record_index = record_index;
    ind->num_index = num_index;
    
    l4c_send_msg_to_bcm(MSG_ID_MMI_L4C_DELETE_PHB_REQ_IND,ind);
    #endif
}   //mtk02514_plutommi_usim
...
void l4c_cc_call_accept_req_ind (kal_uint8 call_id){
    OTA_LOGD(L_CMD, L_V5, "======%d", call_id);
#ifdef __L4C_API__
    OTA_track_l4c_cc_call_accept_req_ind(call_id);
#endif /* __L4C_API__ */
}
...
{
    #ifdef __BCM_PBAP_SERVER__
    l4c_send_msg_to_bcm(MSG_ID_MMI_PHB_STARTUP_FINISH_IND,NULL);
    #endif
}
...
{
#ifdef __L4C_API__
    mmi_sms_mem_full_ind_struct *myMsgPtr = NULL;
    OTA_LOGD(L_CMD, L_V5, "type:%d", type);
    myMsgPtr = (mmi_sms_mem_full_ind_struct*) construct_local_para(sizeof(mmi_sms_mem_full_ind_struct), TD_CTRL);
    myMsgPtr->type = type;
    track_l4c_send_msg_track_mmi(MSG_ID_MMI_SMS_MEM_FULL_IND, myMsgPtr, NULL);
#endif /* __L4C_API__ */
}
void l4c_sms_mem_exceed_lind(kal_uint8 type){OTA_LOGD(L_CMD, L_V4, "%d", type);}
void l4c_sms_mem_available_lind(kal_uint8 type){OTA_LOGD(L_CMD, L_V4, "%d", type);}
...
void l4c_sms_new_msg_text_lind(kal_uint16 index,
								...
								kal_uint16 src_port,
      								...
      								void *msg_header,
			                                   void *data){
    OTA_LOGD(L_CMD, L_V7, "index:%d,dest_port:%d, src_port:%d,mti:%d,dis:%d,storage:%d,total_seg:%d,len%d",
         index, dest_port, src_port, mti, display_type, storage_type, concat_info->total_seg, message_len);
   // LOGH(L_CMD, L_V5, msg_header, sizeof(smsal_deliver_msg_hdr_struct));

    //EllLog("data:%s", data);
    //OTA_LOGH(L_CMD, L_V5, data, message_len + 1);
...
void l4c_sms_startup_read_lind(kal_uint8 sim_status){
    kal_prompt_trace(2, "====l4c_sms_startup_read_lind==sim_status:%d=================",sim_status);
...
{
    kal_prompt_trace(2, "====l4c_sms_ready_lind==result:%d=================",result);
    //MSG_ID_MMI_SMS_READY_IND
    
    if(L4C_OK == result)
    {
        track_l4c_send_msg_track_mmi(MSG_ID_MMI_SMS_READY_IND, NULL, NULL);
    }
}
...
void l4c_sat_run_at_command_lind (sat_run_at_command_ind_struct *local_ptr,
    ...
    kal_prompt_trace(2, "====l4c_sat_run_at_command_lind===================");

} //030602 mtk00468 add for Paul SAT
