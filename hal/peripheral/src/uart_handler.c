...
DCL_STATUS UART_Handler(DCL_DEV dev,DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
    ...
    if(!flag)
    {
      if(kal_get_systicks() > 800) flag = KAL_TRUE;
    }
	...
	switch (cmd)
		...
		case SIO_CMD_GET_BYTES:
            ...
            /* ACTI过来取模拟at指令数据 */
            if(flag && dev == 0 && MOD_ATCI == prCtrlGetBytes->u4OwenrId)
            {   
               /* OTA_LOGD(L_SYS, L_V9, "SIO_CMD_GET_BYTES u4OwenrId:%d(%d), u2RetSize:%d, u2Length:%d", 
                    prCtrlGetBytes->u4OwenrId, 
                    kal_get_active_module_id(), 
                    prCtrlGetBytes->u2RetSize, 
                    prCtrlGetBytes->u2Length);*/
                if(OTA_track_atci_at_command_buf(1, prCtrlGetBytes->puBuffaddr, &prCtrlGetBytes->u2RetSize))
                {
		...
		case SIO_CMD_PUT_BYTES:
        		{
                    //extern void ELL_Funtion_Dummy_atdata_rsq(kal_uint8 type, kal_uint8 *data, U16 *len);
        			UART_CTRL_PUT_BYTES_T* prCtrlPutBytes;
        			prCtrlPutBytes = &(data->rUARTCtrlPUTBYTES);
								   
    #if defined( __AT_CA__)
            /* ACTI返回数据通知MMI */
            if(dev == 0 && MOD_ATCI == prCtrlPutBytes->u4OwenrId && prCtrlPutBytes->u2Length > 0)
            {
                static kal_uint16 i = 0;
                static kal_uint8 buftmp[255] = {0};

#if defined (__CUST_UART1_MUX__)
				kal_uint8 left_length;
#endif /* __CUST_UART1_MUX__ */

                if(!flag)
                {
                    prCtrlPutBytes->u2RetSize = prCtrlPutBytes->u2Length;
                    return;
                }
               /*OTA_LOGD(L_SYS, L_V9, "chengj SIO_CMD_PUT_BYTES u4OwenrId:%d(%d), u2RetSize:%d, u2Length:%d", 
                    prCtrlPutBytes->u4OwenrId, 
                    kal_get_active_module_id(), 
                    prCtrlPutBytes->u2RetSize, 
                    prCtrlPutBytes->u2Length);*/

                if(prCtrlPutBytes->u2Length == 1)
                {
                    if(i < 254)
                    {
                        buftmp[i++] = prCtrlPutBytes->puBuffaddr[0];
                        prCtrlPutBytes->u2RetSize = prCtrlPutBytes->u2Length;
                        if(i > 2 && ((buftmp[i-2] == 0x0D && buftmp[i-1] == 0x0A) || buftmp[i-2] == '>' || buftmp[i-1] == '>'))
                        {
                            OTA_track_at_send_data_rsq(2, buftmp, &i);
                            i = 0;
                        }
							...
							if(i>=5)
							{
								if(buftmp[0]==0xF5)
								{
									left_length=buftmp[4];//buftmp[3]=0 没有长包数据
                ...
                else
                {
                    ...
                    OTA_track_at_send_data_rsq(2, prCtrlPutBytes->puBuffaddr, &prCtrlPutBytes->u2Length);
                }
                prCtrlPutBytes->u2RetSize = prCtrlPutBytes->u2Length;
            }
            else
    ...
    #if defined( __AT_CA__)
            /* ACTI过来取数据长度 */
            if (flag && MOD_ATCI == pUart_CMD_FUNC[dev]->GetOwnerID(dev))
            {
