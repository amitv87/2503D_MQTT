...
/* under construction !*/
...
#if defined(MT6261) || defined(MT2501) || defined(MT2502)
    {
        ...
        long_press_config.value = PMU_LONG_PRESS_SHUTDOWN;
...
void DclPW_PowerInit(void)
{
  ...
  if (KAL_TRUE == g_Hibernation_to_ap_silent_reboot)
    {
        PWRon=ABNRESET;
    }
    else if(factors[PWR_FACTOR_LONG_PRESS_SHUTDOWN])
    {
        PWRon=PWRKEYPWRON;
    }
	else if(factors[PWR_FACTOR_PRECHRPWRON_FLG])
	{
...
/* under construction !*/
	...
	else
	{
		...
		while(1)/*to cover crstal oscillates too late case*/
		{
            ...
            {
                gps_PWRon = PWRon;
                //OTA_log ( "gps_PWRon %d,%d \n" , gps_PWRon, PWRon) ;
                //OTA_track_cust_set_power_on_reason(PWRon);
#if defined(__KEY_POWER_ON__)
                /*按键开机的项目此处不能改变开机类型 -- wangqi 20140122*/
                //PWRon=PWRKEYPWRON;
#else
                PWRon=PWRKEYPWRON;
#endif
                /*开机都以正常按键开机模式启动，
                    否则部分功能有限制       --    chengjun  2012-12-30*/
            }
        #endif /* __GPS_TRACK__ */
...
DCL_STATUS DclPW_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	switch(cmd)
	{
        ...
        case PW_CMD_IS_POWERON_REASON_VALID:
        {
