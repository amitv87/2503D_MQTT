...
#if defined( __KEY_POWER_ON__)
void track_drv_charge_power_key(kal_uint8 keyType, kal_uint8 keyCode)
{
    static kal_bool power_key_long = KAL_FALSE;
    
    OTA_LOGS(" track_charge_power_key:keyType:%d,keyCode:%d,power_key_log:%d %d", keyType,keyCode,power_key_long,DEVICE_KEY_POWER);
    drv_trace2(TRACE_GROUP_7, KBD_PUT_DATA_TO_BUFFER, keyType, keyCode);
    drv_trace2(TRACE_GROUP_7, KBD_PUT_DATA_TO_BUFFER, DEVICE_KEY_POWER, 0xff);

    if(DEVICE_KEY_POWER == keyCode)
    {
        if(kbd_onekey_longpress == keyType)
        {
            //power_key_long = KAL_TRUE;
        //}
        //else if(kbd_onekey_release == keyType && power_key_long)
        //{
            l4cuem_power_reset((kal_uint8)0/*NORMAL_RESET*/);
            OTA_LOGS("track_charge_power_key:reset----------------------");
    //drv_trace2(TRACE_GROUP_8, KBD_PUT_DATA_TO_BUFFER, 0xff, 0xff);
...
drv_trace2(TRACE_GROUP_7, KBD_PUT_DATA_TO_BUFFER, event, key);
#if defined( __KEY_POWER_ON__)
    if(INT_Get_PowerOn_Type() == CHRPWRON)
    {
        track_drv_charge_power_key(event, key);
    }
#endif /* __KEY_POWER_ON__ */
